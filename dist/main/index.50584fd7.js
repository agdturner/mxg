...
var _guiMoleculesJs = require("./gui_molecules.js");
var _guiReactionsJs = require("./gui_reactions.js");
var _guiConditionsListJs = require("./gui_conditionsList.js");
var _guiModelParametersJs = require("./gui_modelParameters.js");
 ...
 */ document.addEventListener("DOMContentLoaded", ()=>{
    ...
    */ // Initialise mesmer.
    ...
    mesmer = new (0, _mesmerJs.Mesmer)(mesmerAttributes);
    // Create the menu.
    ...
    let mlDivID = addRID((0, _mesmerJs.MoleculeList).tagName);
    let mlDiv = (0, _htmlJs.createDiv)(mlDivID);
    moleculesDiv.appendChild(mlDiv);
    // Create collapsible content.
    let mlcDiv = (0, _htmlJs.getCollapsibleDiv)(mlDivID, moleculesDiv, null, mlDiv, (0, _mesmerJs.MoleculeList).tagName, boundary1, level0);
    // Add add molecule button.
    let mb = (0, _guiMoleculesJs.getAddMoleculeButton)(mlDiv, molecules);
    // Add add from library button.
    let lb = (0, _guiMoleculesJs.getAddFromLibraryButton)(mlDiv, mb, molecules);
    // Reactions.
    let reactionsDiv = document.getElementById(reactionsDivID);
    let rlDivID = addRID((0, _mesmerJs.ReactionList).tagName);
    let rlDiv = (0, _htmlJs.createDiv)(rlDivID);
    reactionsDiv.appendChild(rlDiv);
    // Create collapsible content.
    let rlcDiv = (0, _htmlJs.getCollapsibleDiv)(rlDivID, reactionsDiv, null, rlDiv, (0, _mesmerJs.ReactionList).tagName, boundary1, level0);
    // Add add reaction button.
    let rb = (0, _guiReactionsJs.getAddReactionButton)(rlDiv, reactions, molecules);
    // Reactions Diagram.
    ...
    let bDivId = addRID(rdDivID, (0, _htmlJs.s_button) + "s");
    let bDiv = (0, _htmlJs.createDiv)(bDivId);
    rdDiv.appendChild(bDiv);
    let pbID = addRID(bDivId, (0, _htmlJs.s_button));
    let pb = (0, _htmlJs.createButton)("Pop into a new Window", pbID);
    bDiv.appendChild(pb);
    ...
    let cdlDivID = addRID((0, _conditionsJs.Conditions).tagName);
    let cdlDiv = (0, _htmlJs.createDiv)(cdlDivID);
    conditionsDiv.appendChild(cdlDiv);
    // Create collapsible content.
    let cdlcDiv = (0, _htmlJs.getCollapsibleDiv)(cdlDivID, conditionsDiv, null, cdlDiv, "ConditionsList", boundary1, level0);
    // Model Parameters.
    let modelParametersDiv = document.getElementById(modelParametersDivID);
    let mplDivID = addRID((0, _modelParametersJs.ModelParameters).tagName, "list");
    let mplDiv = (0, _htmlJs.createDiv)(mplDivID);
    modelParametersDiv.appendChild(mplDiv);
    // Create collapsible content.
    let mplcDiv = (0, _htmlJs.getCollapsibleDiv)(mplDivID, modelParametersDiv, null, mplDiv, "ModelParametersList", boundary1, level0);
    // Control.
    let controlDiv = document.getElementById(controlDivID);
    let clDivID = addRID((0, _controlJs.Control).tagName);
    let clDiv = (0, _htmlJs.createDiv)(clDivID);
    controlDiv.appendChild(clDiv);
    // Create collapsible content.
    let controlcDiv = (0, _htmlJs.getCollapsibleDiv)(clDivID, controlDiv, null, clDiv, "ControlList", boundary1, level0);
    // MetadataList.
    let metadataListDiv = document.getElementById(metadataListDivID);
    let mdDivID = addRID((0, _metadataJs.MetadataList).tagName);
    let mdDiv = (0, _htmlJs.createDiv)(mdDivID);
    metadataListDiv.appendChild(mdDiv);
    // Create collapsible content.
    let mdcDiv = (0, _htmlJs.getCollapsibleDiv)(mdDivID, metadataListDiv, null, mdDiv, (0, _metadataJs.MetadataList).tagName, boundary1, level0);
    // Analysis.
    let analysisDiv = document.getElementById(analysisDivID);
    let aDivID = addRID((0, _analysisJs.Analysis).tagName);
    let aDiv = (0, _htmlJs.createDiv)(aDivID);
    analysisDiv.appendChild(aDiv);
    // Create collapsible content.
    let acDiv = (0, _htmlJs.getCollapsibleDiv)(aDivID, analysisDiv, null, aDiv, (0, _analysisJs.Analysis).tagName, boundary1, level0);
    // XML.
    let xmlDiv = document.getElementById(xmlDivID);
    let xDivID = addRID(s_xml, 2);
    let xDiv = (0, _htmlJs.createDiv)(xDivID);
    xmlDiv.appendChild(xDiv);
    // Create collapsible content.
    let xcDiv = (0, _htmlJs.getCollapsibleDiv)(xDivID, xmlDiv, null, xDiv, s_xml, boundary1, level0);
});
/**
 * Create the title input.
 */ function createTitle(title, attributes) {
    let titleNode = new (0, _mesmerJs.Title)(attributes, title);
    ...
    }, title, (0, _mesmerJs.Title).tagName);
    lwi.id = lwiId;
 ...
 */ function parse(xml) {
    ...
    if (xml_title.length > 0) {
        if (xml_title.length > 1) console.warn("Multiple " + (0, _mesmerJs.Title).tagName + " tags found, using the first.");
        title = xml_title[0].childNodes[0].nodeValue.trim();
        attributes = (0, _xmlJs.getAttributes)(xml_title[0]);
    } else {
        title = filename;
        console.warn("No " + (0, _mesmerJs.Title).tagName + " tag found, using the filename: " + filename + " as the title.");
        attributes = new Map();
    }
    createTitle(title, attributes);
    // Molecules.
    ...
    remove(mlDivID);
    // Create collapsible content.
    let mlcDiv = (0, _htmlJs.getCollapsibleDiv)(mlDivID, mlDiv, null, (0, _guiMoleculesJs.processMoleculeList)(xml, molecules), (0, _mesmerJs.MoleculeList).tagName, boundary1, level0);
    ...
    remove(rlDivID);
    // Create collapsible content.
    let rlcDiv = (0, _htmlJs.getCollapsibleDiv)(rlDivID, rlDiv, null, (0, _guiReactionsJs.processReactionList)(xml, reactions), (0, _mesmerJs.ReactionList).tagName, boundary1, level0);
    // Reactions Diagram.
    let rddDiv = document.getElementById(reactionsDiagramDivID);
    ...
    remove(cdlDivID);
    // Create collapsible content.
    let cdlcDiv = (0, _htmlJs.getCollapsibleDiv)(cdlDivID, cdlDiv, null, (0, _guiConditionsListJs.processConditions)(xml, conditionsIDs, molecules), "ConditionsList", boundary1, level0);
    // Model Parameters.
    ...
    remove(mplDivID);
    // Create collapsible content.
    let mplcDiv = (0, _htmlJs.getCollapsibleDiv)(mplDivID, mplDiv, null, (0, _guiModelParametersJs.processModelParameters)(xml, modelParametersIDs), "ModelParametersList", boundary1, level0);
    // Control.
    ...
    remove(clDivID);
    // Create collapsible content.
    let controlcDiv = (0, _htmlJs.getCollapsibleDiv)(clDivID, clDiv, null, (0, _guiControlListJs.processControl)(xml, controlIDs), "ControlList", boundary1, level0);
    // MetadataList.
    ...
    remove(mdDivID);
    // Create collapsible content.
    let mdcDiv = (0, _htmlJs.getCollapsibleDiv)(mdDivID, mdDiv, null, processMetadataList(xml), (0, _metadataJs.MetadataList).tagName, boundary1, level0);
    // Analysis.
...
function addPropertyScalarNumber(attributes, iDs, value, units, pl, p, plDiv, boundary) {
    ...
    console.log("unitsID " + addRID(id, (0, _moleculeJs.PropertyScalarNumber).s_units));
    addAnyUnits(units, attributes, div, div.querySelector(s_input), (0, _utilJs.getID)(id, (0, _moleculeJs.PropertyScalarNumber).s_units), p.dictRef, boundary, boundary);
 ...
 */ function processNumberArray(id, name, pa, getter, setter, remover, marginComponent, margin) {
    ...
    let buttonTextContentSelected = name + sy_selected;
    let buttonTextContentDeselected = name + sy_deselected;
    let button = (0, _htmlJs.createButton)(buttonTextContentDeselected, addRID(id, (0, _htmlJs.s_button)), marginComponent);
    div.appendChild(button);
    button.classList.add(s_optionOn);
    button.classList.add(s_optionOff);
    let inputId = addRID(id, name, s_input);
...
function processPropertyScalarNumber(pl, p, units, molecule, element, plDiv, boundary, level) {
    ...
    let pIDs = new Set();
    // PropertyScalar.
    let scalarNodes = element.getElementsByTagName((0, _moleculeJs.PropertyScalarNumber).tagName);
    if (scalarNodes.length > 0) {
        if (scalarNodes.length != 1) throw new Error("Expecting 1 " + (0, _moleculeJs.PropertyScalarNumber).tagName + " but finding " + scalarNodes.length + "!");
        ...
        let ps = new (0, _moleculeJs.PropertyScalarNumber)(psAttributes, value);
        p.setProperty(ps);
        ps.setValue = (function(value) {
            ps.value = value;
            if (p.dictRef == (0, _moleculeJs.ZPE).dictRef) // Update the molecule energy diagram.
        ...
        addAnyUnits(units, psAttributes, div, div.querySelector(s_input), addRID(plDiv.id, p.dictRef, (0, _moleculeJs.PropertyScalarNumber).s_units), p.dictRef, boundary, boundary);
        plDiv.appendChild(div);
    } else {
        // PropertyArray.
        ...
        if (arrayNodes.length > 0) {
            ...
            if (inputString != "") {
                ...
                addAnyUnits(units, paAttributes, div, div.querySelector(s_textarea), addRID(plDiv.id, p.dictRef, (0, _moleculeJs.PropertyArray).s_units), p.dictRef, boundary, boundary);
                plDiv.appendChild(div);
            }
        } else {
            // PropertyMatrix.
            ...
            } else throw new Error("Expecting " + (0, _moleculeJs.PropertyScalarNumber).tagName + ", " + (0, _moleculeJs.PropertyArray).tagName + " or " + (0, _moleculeJs.PropertyMatrix).tagName + " but finding none!");
        }
    }
}
function processPropertyScalarString(pl, p, molecule, element, plDiv, boundary, level) {
    ...
    let scalarNodes = element.getElementsByTagName((0, _moleculeJs.PropertyScalarString).tagName);
    if (scalarNodes.length > 0) {
        if (scalarNodes.length != 1) throw new Error("Expecting 1 " + (0, _moleculeJs.PropertyScalarString).tagName + " but finding " + scalarNodes.length + "!");
        let inputString = (0, _xmlJs.getInputString)(scalarNodes[0]);
        let psAttributes = (0, _xmlJs.getAttributes)(scalarNodes[0]);
        // Add PropertyScalarNumber.
        let ps = new (0, _moleculeJs.PropertyScalarString)(psAttributes, inputString);
        p.setProperty(ps);
        ps.setValue = (function(value) {
            ps.value = value;
        ...
        plDiv.appendChild(div);
    } else console.log("Expecting " + (0, _moleculeJs.PropertyScalarString).tagName + " but finding none!");
}
 ...
 */ function processMetadataList(xml) {
    ...
    let mlDiv = (0, _htmlJs.createDiv)(addRID((0, _metadataJs.MetadataList).tagName, 0), boundary1);
 ...
 */ function processAnalysis(xml) {
    ...
    let aDivID = addRID((0, _analysisJs.Analysis).tagName, 0);
    ...
    if (xml_as.length > 0) {
        ...
        if (xml_d.length > 0) {
            if (xml_d.length == 1) {
                ...
                let dDiv = (0, _htmlJs.createDiv)(addRID(aDivID, (0, _mesmerJs.Description).tagName), level1);
        ...
        let elDivID = addRID(aDivID, (0, _analysisJs.EigenvalueList).tagName);
        ...
        let plDivID = addRID(aDivID, (0, _analysisJs.PopulationList).tagName);
        ...
        for(let i = 0; i < xml_pl.length; i++){
            ...
            let plDivID = addRID(aDiv.id, (0, _analysisJs.PopulationList).tagName, i.toString());
            // Create a new collapsible div for the EigenvalueList.
        ...
        // me:rateList.
        let xml_rl = xml_as[0].getElementsByTagName((0, _analysisJs.RateList).tagName);
        // Create a new collapsible div for the RateLists.
        let rlDivID = addRID(aDivID, (0, _analysisJs.RateList).tagName);
        let rlDiv = (0, _htmlJs.createDiv)(rlDivID, level1);
        let rlcDiv = (0, _htmlJs.getCollapsibleDiv)(rlDivID, aDiv, null, rlDiv, (0, _analysisJs.RateList).tagName + "s", boundary1, level1);
        if (xml_rl.length > 0) {
            // Create Table.
            let tableDiv = (0, _htmlJs.createDiv)(addRID(rlDivID, s_table), boundary1);
            ...
            for(let i = 0; i < xml_rl.length; i++){
                let rl_attributes = (0, _xmlJs.getAttributes)(xml_rl[i]);
                let values = [];
                ...
                }*/ let rl = new (0, _analysisJs.RateList)(rl_attributes);
                a.addRateList(rl);
                /*
                ...
                */ // "me:firstOrderRate".
                let xml_for = xml_rl[i].getElementsByTagName((0, _analysisJs.FirstOrderRate).tagName);
                if (xml_for.length > 0) //console.log("me:firstOrderRate length " + xml_for.length);
                for(let j = 0; j < xml_for.length; j++){
                    let forate_attributes = (0, _xmlJs.getAttributes)(xml_for[j]);
                    if (i == 0) {
                    ...
                    values.push(s);
                    let forate = new (0, _analysisJs.FirstOrderRate)(forate_attributes, new (0, _bigJsDefault.default)(s));
                    rl.addFirstOrderRate(forate);
                }
                // "me:firstOrderLoss".
                let xml_fol = xml_rl[i].getElementsByTagName((0, _analysisJs.FirstOrderLoss).tagName);
                ...
                for(let j = 0; j < xml_fol.length; j++){
                    ...
                    values.push(s);
                    let fol = new (0, _analysisJs.FirstOrderLoss)(fol_attributes, new (0, _bigJsDefault.default)(s));
    ...
    if (canvas != null) {
        ...
        reactions.forEach(function(reaction, id) {
            ...
            if (reactantsLabel != undefined) {
                ...
                let energy = reaction.getReactantsEnergy(getMolecule, molecules);
                ...
                if (orders.has(productsLabel)) {
                    ...
                    if (reactionTransitionStates != undefined) {
                        reactionTransitionStates.forEach(function(ts) {
                            ...
                            orders.set(ref, i);
                            energy = getMolecule(ref, molecules).getEnergy() ?? big0;
                            energyMin = (0, _utilJs.min)(energyMin, energy);
                            energyMax = (0, _utilJs.max)(energyMax, energy);
        ...
        reorders.forEach(function(value) {
            ...
            let energyRescaled = (0, _utilJs.rescale)(energyMin.toNumber(), energyRange, 0, rdcHeight, energy);
            // Get text width.
        ...
        reactants.forEach(function(value) {
            let energy = (0, _utilJs.get)(energies, value);
            let energyRescaled = (0, _utilJs.rescale)(energyMin.toNumber(), energyRange, 0, originalCanvasHeight, energy);
...
function saveXML() {
    if (mesmer == null) {
         ...
         * metadataList
         * analysis
         */ let mesmerOrdered = new (0, _mesmerJs.Mesmer)(mesmer.attributes);
        mesmerOrdered.setTitle(mesmer.getTitle());
        if (molecules != undefined) mesmerOrdered.setMoleculeList(new (0, _mesmerJs.MoleculeList)(new Map(), Array.from(molecules.values())));
        if (reactions != undefined) mesmerOrdered.setReactionList(new (0, _mesmerJs.ReactionList)(new Map(), Array.from(reactions.values())));
        ...
        console.log("saveXML");
        const pad = "  ";
        let xmlData = (0, _mesmerJs.Mesmer).header + mesmerOrdered.toXML(pad, "");
...
},{"./util.js":"f0Rnl","./xml.js":"7znDa","./molecule.js":"ahQNx","./gui_menu.js":"2khyJ","./html.js":"aLPSL","./canvas.js":"hoJRr","./conditions.js":"aksKl","./modelParameters.js":"kQHfz","./control.js":"Qx5gu","./mesmer.js":"kMp4Q","big.js":"91nMZ","./analysis.js":"53wyH","./metadata.js":"aKNnu","./defaults.js":"d6DU0","./gui_molecules.js":"cZnII","./gui_reactions.js":"iXoY9","./gui_conditionsList.js":"i75Ii","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3","./gui_modelParameters.js":"cN8Dd","./gui_controlList.js":"9Xsct"}],"f0Rnl":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
/**
...
},{"big.js":"91nMZ","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"91nMZ":[function(require,module,exports) {
(function(GLOBAL) {
    "use strict";
...
class NumberArrayNode extends TagWithAttributes {
     ...
     * @returns An XML representation.
     */ toXML(padding) {
        return getTag((0, _util.bigArrayToString)(this.values, this.delimiter), this.tagName, this.attributes, padding, false);
    }
}
...
},{"./html":"aLPSL","./util":"f0Rnl","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"aLPSL":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
},{"./util":"f0Rnl","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"ahQNx":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
/**
 * Atom data.
...
class Atom extends (0, _xmlJs.TagWithAttributes) {
     ...
     */ getX3() {
        ...
        if (x3 != undefined) return new (0, _bigJsDefault.default)(x3);
    }
    /**
     ...
     */ getY3() {
        ...
        if (y3 != undefined) return new (0, _bigJsDefault.default)(y3);
    }
    /**
     ...
     */ getZ3() {
        ...
        if (z3 != undefined) return new (0, _bigJsDefault.default)(z3);
    }
    /**
...
class DeltaEDown extends (0, _rangeJs.RangeNode) {
    static{
        /**
     * The tag name.
...
class DeltaEDownTExponent extends (0, _rangeJs.RangeNode) {
    static{
        /**
     * The tag name.
...
class DeltaEDownLinEne extends (0, _rangeJs.RangeNode) {
    static{
        /**
     * The tag name.
...
class ThermoValue extends (0, _xmlJs.TagWithAttributes) {
     ...
     */ getT() {
        return new (0, _bigJsDefault.default)(this.attributes.get(ThermoValue.s_T));
    }
    /**
     ...
     */ getH() {
        return new (0, _bigJsDefault.default)(this.attributes.get(ThermoValue.s_H));
    }
    /**
     ...
     * @returns The entropy.
     */ getS() {
        return new (0, _bigJsDefault.default)(this.attributes.get(ThermoValue.s_S));
    }
    /**
     * @param S The entropy.
     *
     ...
     */ getG() {
        return new (0, _bigJsDefault.default)(this.attributes.get(ThermoValue.s_G));
    }
    /**
     ...
     */ getCp() {
        return new (0, _bigJsDefault.default)(this.attributes.get(ThermoValue.s_Cp));
    }
    /**
...
},{"big.js":"91nMZ","./range.js":"4A23j","./util.js":"f0Rnl","./xml.js":"7znDa","./mesmer.js":"kMp4Q","./metadata.js":"aKNnu","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"4A23j":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
/**
...
class RangeNode extends (0, _xml.NumberNode) {
     ...
     */ getLower() {
        ...
        if (lower != undefined) return new (0, _bigJsDefault.default)(lower);
    }
    /**
     ...
     */ getUpper() {
        ...
        if (upper != undefined) return new (0, _bigJsDefault.default)(upper);
    }
    /**
     ...
     */ getStepsize() {
        ...
        if (stepsize != undefined) return new (0, _bigJsDefault.default)(stepsize);
    }
    /**
...
<?xml-stylesheet type='text/xsl' href='../../mesmer1.xsl' media='screen'?>`;
     ...
     */ constructor(attributes, title, moleculeList, reactionList, conditionss, modelParameterss, controls, metadataList, analysis){
        ...
        if (modelParameterss != undefined) modelParameterss.forEach((modelParameters)=>{
            this.index.set((0, _modelParametersJs.ModelParameters).tagName + modelParameters.id, this.nodes.size);
     ...
     * @param modelParameters The ModelParameters.
     */ addModelParameters(modelParameters) {
        let id = (0, _modelParametersJs.ModelParameters).tagName + modelParameters.id;
        let i = this.index.get(id);
     ...
     */ removeModelParameters(modelParametersID) {
        ...
        if (i != undefined) {
            this.nodes.delete(i);
            this.index.delete((0, _modelParametersJs.ModelParameters).tagName + modelParametersID);
...
},{"./analysis.js":"53wyH","./conditions.js":"aksKl","./control.js":"Qx5gu","./metadata.js":"aKNnu","./modelParameters.js":"kQHfz","./xml.js":"7znDa","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"53wyH":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
/**
        ...
        */ // Process the XML "molecule" elements.
        ...
        for(let i = 0; i < xml_msl; i++){
            ...
            if (xml_mls.length > 0) {
                ...
                let xml_ms = xml_mls[0].getElementsByTagName((0, _metadata.Metadata).tagName);
            ...
            if (xml_aas.length == 1) {
                ...
                m.setAtoms(aa);
                for(let j = 0; j < xml_as.length; j++)aa.addAtom(new (0, _molecule.Atom)((0, _xml.getAttributes)(xml_as[j]), m));
            ...
            if (xml_bas.length > 0) {
                ...
                let ba = new (0, _molecule.BondArray)((0, _xml.getAttributes)(xml_bas[0]));
                for(let j = 0; j < xml_bs.length; j++)ba.addBond(new (0, _molecule.Bond)((0, _xml.getAttributes)(xml_bs[j]), m));
                m.setBonds(ba);
                ...
                if (xml_bonds.length > 0) {
                    ...
                    let ba = new (0, _molecule.BondArray)(new Map());
...
},{"./util":"f0Rnl","./xml":"7znDa","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"cZnII":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
/**
 * 
...
var _utilJs = require("./util.js");
var _xmlJs = require("./xml.js");
function getAddMoleculeButton(mlDiv, molecules) {
    ...
    addMoleculeButton.addEventListener("click", ()=>{
        let mid = "Kr";
        let m = new (0, _moleculeJs.Molecule)(new Map(), mid);
        mid = setMoleculeID(mid, m, molecules);
        (0, _appJs.addMolecule)(m, molecules);
        m.setAtoms(new (0, _moleculeJs.AtomArray)(new Map()));
        m.setBonds(new (0, _moleculeJs.BondArray)(new Map()));
        let moleculeDivID = (0, _appJs.addRID)((0, _moleculeJs.Molecule).tagName, mid);
        ...
        let mcDiv = (0, _htmlJs.getCollapsibleDiv)(mcDivID, mlDiv, addMoleculeButton, moleculeDiv, mid, (0, _appJs.boundary1), (0, _appJs.level1));
        // Add the molecule to the BathGas select elements.
        (0, _appJs.addOptionByClassName)((0, _conditionsJs.BathGas).tagName, mid);
        // Add edit Name button.
        addEditIDButton(m, mcDiv.querySelector((0, _htmlJs.s_button)), moleculeDiv, (0, _appJs.level1));
        // Description
        moleculeDiv.appendChild(processDescription((0, _appJs.addRID)(moleculeDivID, (0, _appJs.s_description)), m.getDescription.bind(m), m.setDescription.bind(m), (0, _appJs.boundary1), (0, _appJs.level1)));
        // Create collapsible AtomArray HTMLDivElement.
        let aaDivID = (0, _appJs.addRID)(moleculeDivID, (0, _moleculeJs.AtomArray).tagName);
        let aaDiv = (0, _htmlJs.createDiv)(aaDivID);
        let aacDivID = (0, _appJs.addRID)(aaDivID, (0, _appJs.s_container));
        let aacDiv = (0, _htmlJs.getCollapsibleDiv)(aacDivID, moleculeDiv, null, aaDiv, (0, _moleculeJs.AtomArray).tagName, (0, _appJs.boundary1), (0, _appJs.level1));
        aaDiv.appendChild(getAddAtomButton(m, aaDiv, (0, _moleculeJs.Atom).tagName, (0, _appJs.boundary1), (0, _appJs.level1)));
        // Create collapsible BondArray HTMLDivElement.
        let baDivID = (0, _appJs.addRID)(moleculeDivID, (0, _moleculeJs.BondArray).tagName);
        let baDiv = (0, _htmlJs.createDiv)(baDivID);
        let bacDivID = (0, _appJs.addRID)(baDivID, (0, _appJs.s_container));
        let bacDiv = (0, _htmlJs.getCollapsibleDiv)(bacDivID, moleculeDiv, null, baDiv, (0, _moleculeJs.BondArray).tagName, (0, _appJs.boundary1), (0, _appJs.level1));
        baDiv.appendChild(getAddBondButton(m, baDiv, (0, _moleculeJs.Bond).tagName, (0, _appJs.boundary1), (0, _appJs.level1)));
        create3DViewer(m, moleculeDiv, (0, _appJs.boundary1), (0, _appJs.level1));
        // Create collapsible Properties HTMLDivElement.
        let plDivID = (0, _appJs.addRID)(moleculeDivID, (0, _moleculeJs.PropertyList).tagName);
        let plDiv = (0, _htmlJs.createDiv)(plDivID);
        let plcDivID = (0, _appJs.addRID)(plDivID, (0, _appJs.s_container));
        let plcDiv = (0, _htmlJs.getCollapsibleDiv)(plcDivID, moleculeDiv, null, plDiv, (0, _moleculeJs.PropertyList).tagName, (0, _appJs.boundary1), (0, _appJs.level1));
...
function getAddFromLibraryButton(mlDiv, mb, molecules) {
    ...
    addFromLibraryButton.addEventListener("click", ()=>{
        ...
        let selectDiv = (0, _htmlJs.createDiv)((0, _appJs.addRID)(selectDivID), (0, _appJs.level1));
        ...
        let selectID = (0, _utilJs.getID)(selectDivID, (0, _htmlJs.s_select));
        ...
        select.classList.add((0, _moleculeJs.Molecule).tagName);
        ...
        select.addEventListener("change", (event)=>{
            ...
            let moleculeDivID = (0, _appJs.addRID)((0, _moleculeJs.Molecule).tagName, molecules.size);
            let moleculeDiv = (0, _htmlJs.createDiv)(moleculeDivID);
            // Create collapsible Molecule HTMLDivElement.
            let mcDivID = (0, _appJs.addRID)(moleculeDivID, (0, _appJs.s_container));
            let mcDiv = (0, _htmlJs.getCollapsibleDiv)(mcDivID, mlDiv, mb, moleculeDiv, molecule.getLabel(), (0, _appJs.boundary1), (0, _appJs.level1));
            // Add the molecule to the BathGas select elements.
            (0, _appJs.addOptionByClassName)((0, _conditionsJs.BathGas).tagName, molecule.getID());
            // Add edit Name button.
            addEditIDButton(molecule, mcDiv.querySelector((0, _htmlJs.s_button)), moleculeDiv, (0, _appJs.level1));
            // Description
            moleculeDiv.appendChild(processDescription((0, _appJs.addRID)(moleculeDivID, (0, _appJs.s_description)), molecule.getDescription.bind(molecule), molecule.setDescription.bind(molecule), (0, _appJs.boundary1), (0, _appJs.level1)));
            // Create collapsible MetadataList HTMLDivElement.
            let mlistDivID = (0, _appJs.addRID)(moleculeDivID, (0, _metadataJs.MetadataList).tagName);
            let mlistDiv = (0, _htmlJs.createDiv)(mlistDivID, (0, _appJs.level1));
            let mlistcDivID = (0, _appJs.addRID)(mlistDivID, (0, _appJs.s_container));
            let mlistcDiv = (0, _htmlJs.getCollapsibleDiv)(mlistcDivID, moleculeDiv, null, mlistDiv, (0, _metadataJs.MetadataList).tagName, (0, _appJs.boundary1), (0, _appJs.level1));
            // Add metadata.
            ...
            if (aa != undefined) aa.atoms.forEach((a)=>{
                aaDiv.appendChild(addAtom(molecule, aaDivID, molecule.getAtoms(), a, (0, _appJs.boundary1), (0, _appJs.level1)));
            });
            aaDiv.appendChild(getAddAtomButton(molecule, aaDiv, (0, _moleculeJs.Atom).tagName, (0, _appJs.boundary1), (0, _appJs.level1)));
            // Create collapsible BondArray HTMLDivElement.
            let baDivID = (0, _appJs.addRID)(moleculeDivID, (0, _moleculeJs.BondArray).tagName);
            let baDiv = (0, _htmlJs.createDiv)(baDivID);
            let bacDivID = (0, _appJs.addRID)(baDivID, (0, _appJs.s_container));
            ...
            if (ba != undefined) molecule.getBonds().bonds.forEach((b)=>{
                ...
                baDiv.appendChild(addBond(molecule, baDivID, aa.atoms, molecule.getBonds(), b, (0, _appJs.boundary1), (0, _appJs.level1)));
            });
            baDiv.appendChild(getAddBondButton(molecule, baDiv, (0, _moleculeJs.Bond).tagName, (0, _appJs.boundary1), (0, _appJs.level1)));
            create3DViewer(molecule, moleculeDiv, (0, _appJs.boundary1), (0, _appJs.level1));
            // Create collapsible Properties HTMLDivElement.
            let plDivID = (0, _appJs.addRID)(moleculeDivID, (0, _moleculeJs.PropertyList).tagName);
            let plDiv = (0, _htmlJs.createDiv)(plDivID);
            let plcDivID = (0, _appJs.addRID)(plDivID, (0, _appJs.s_container));
 ...
 */ function addEditIDButton(molecule, button, moleculeDiv, level) {
    ...
    let editNameButtonID = (0, _appJs.addRID)(moleculeDiv.id, s_editName, (0, _htmlJs.s_button));
    ...
    editNameButton.addEventListener("click", ()=>{
        ...
        (0, _appJs.removeOptionByClassName)((0, _conditionsJs.BathGas).tagName, molecule.getID());
 ...
 */ function processDescription(id, getter, setter, marginComponent, marginDiv) {
    ...
    let button = (0, _htmlJs.createButton)(buttonTextContentDeselected, (0, _appJs.addRID)(id, (0, _htmlJs.s_button)), marginComponent);
    div.appendChild(button);
    button.classList.add((0, _appJs.s_optionOn));
    button.classList.add((0, _appJs.s_optionOff));
    let inputId = (0, _appJs.addRID)(id, (0, _appJs.s_description), (0, _appJs.s_input));
    let value = getter();
    if (value == undefined) {
        button.textContent = buttonTextContentDeselected;
 ...
 */ function getAddAtomButton(molecule, aaDiv, typeID, boundary, level) {
    ...
    let button = (0, _htmlJs.createButton)((0, _appJs.s_Add_sy_add), (0, _appJs.addRID)(aaDiv.id, "Add" + typeID + "Button"), level);
    button.addEventListener("click", ()=>{
        let attributes = new Map();
 ...
 */ function addAtom(molecule, aaDivID, aa, a, boundary, level) {
    ...
    let aDivID = (0, _appJs.addRID)(aaDivID, aID);
    ...
    (0, _appJs.addOptionByClassName)((0, _moleculeJs.Bond).s_atomRefs2, aID);
    return aDiv;
}
/**
 ...
 */ function removeAtom(molecule, aID, aIDs) {
    ...
    (0, _appJs.removeOptionByClassName)((0, _moleculeJs.Bond).s_atomRefs2, aID);
    molecule.getBonds().bonds.forEach((bond)=>{
        ...
        if (atomRefs[0] == atomRefs[1]) {
            ...
            (0, _appJs.removeOptionByClassName)((0, _moleculeJs.Bond).tagName, bondId);
 ...
 */ function processElementType(a, aDiv, aIDs, first, margin) {
    ...
    let selectTypes = (0, _mesmerJs.Mesmer).elementTypes;
    // Select.
    ...
    let id = (0, _appJs.addRID)(aDiv.id, (0, _moleculeJs.Atom).s_elementType);
    aIDs.add(id);
    let lws = (0, _htmlJs.createLabelWithSelect)((0, _moleculeJs.Atom).s_elementType, selectTypes, (0, _moleculeJs.Atom).s_elementType, elementType, id, margin, margin);
    let select = lws.querySelector("select");
    select.addEventListener("change", (event)=>{
        let target = event.target;
 ...
 */ function processCoordinates(a, aDiv, aIDs, marginComponent, margin) {
    let id;
    id = (0, _appJs.addRID)(aDiv.id, (0, _moleculeJs.Atom).s_x3);
    aIDs.add(id);
    aDiv.appendChild((0, _appJs.processNumber)(id, aIDs, (0, _moleculeJs.Atom).s_x3, a.getX3.bind(a), a.setX3.bind(a), a.removeX3, marginComponent, margin));
    id = (0, _appJs.addRID)(aDiv.id, (0, _moleculeJs.Atom).s_y3);
    aIDs.add(id);
    aDiv.appendChild((0, _appJs.processNumber)(id, aIDs, (0, _moleculeJs.Atom).s_y3, a.getY3.bind(a), a.setY3.bind(a), a.removeY3, marginComponent, margin));
    id = (0, _appJs.addRID)(aDiv.id, (0, _moleculeJs.Atom).s_z3);
    aIDs.add(id);
    aDiv.appendChild((0, _appJs.processNumber)(id, aIDs, (0, _moleculeJs.Atom).s_z3, a.getZ3.bind(a), a.setZ3.bind(a), a.removeZ3, marginComponent, margin));
}
/**
 ...
 */ function getAddBondButton(molecule, baDiv, typeID, boundary, level) {
    ...
    let id = (0, _appJs.addRID)(baDiv.id, typeID, (0, _htmlJs.s_button));
 ...
 */ function addBond(molecule, baDivID, atoms, ba, b, boundary, level) {
    ...
    (0, _appJs.addOptionByClassName)((0, _moleculeJs.Bond).tagName, bID);
    return bDiv;
}
/**
 ...
 */ function processAtomRefs2(molecule, bDiv, bond, margin) {
    ...
    let id = (0, _utilJs.getID)(bDiv.id, (0, _moleculeJs.Bond).s_atomRefs2);
    //bIDs.add(id);
    let atomRefs2 = bond.getAtomRefs2();
    let atomRefs = atomRefs2.split(" ");
    ...
    //bIDs.add(alwsID);
    let alws = (0, _htmlJs.createLabelWithSelect)((0, _moleculeJs.Bond).s_atomRefs2 + "[0]", atomRefOptions, (0, _moleculeJs.Atom).tagName, atomRefs[0], alwsID, margin, margin);
    let aselect = alws.querySelector("select");
    aselect.classList.add((0, _moleculeJs.Bond).s_atomRefs2);
    aselect.addEventListener("change", (event)=>{
        let target = event.target;
        let atomRefs2 = target.value + " " + atomRefs[1];
        console.log((0, _moleculeJs.Bond).s_atomRefs2 + " changed to " + atomRefs2);
    ...
    //bIDs.add(blwsID);
    let blws = (0, _htmlJs.createLabelWithSelect)((0, _moleculeJs.Bond).s_atomRefs2 + "[1]", atomRefOptions, (0, _moleculeJs.Atom).tagName, atomRefs[1], blwsID, margin, margin);
    let bselect = blws.querySelector("select");
    bselect.classList.add((0, _moleculeJs.Bond).s_atomRefs2);
 ...
 */ function processOrder(bondDiv, bond, margin) {
    ...
    let id = (0, _utilJs.getID)(bondDiv.id, (0, _moleculeJs.Bond).s_order);
    let div = (0, _htmlJs.createFlexDiv)(undefined, margin);
    bondDiv.appendChild(div);
    let buttonTextContentSelected = (0, _moleculeJs.Bond).s_order + (0, _appJs.sy_selected);
    let buttonTextContentDeselected = (0, _moleculeJs.Bond).s_order + (0, _appJs.sy_deselected);
 ...
 */ function addOrder(div, bond, id, value, boundary) {
    ...
    let select = (0, _htmlJs.createSelectElement)((0, _moleculeJs.Bond).orderOptions, (0, _moleculeJs.Bond).s_order, valueString, id, boundary);
    select.addEventListener("change", (event)=>{
        let target = event.target;
        bond.setOrder(parseFloat(target.value));
        console.log((0, _moleculeJs.Bond).s_order + " changed from " + valueString + " to " + target.value);
        (0, _htmlJs.resizeSelectElement)(target);
    });
 ...
 */ function processUseSineTerms(hrpDiv, hrp, margin) {
    let id = (0, _appJs.addRID)(hrpDiv.id, (0, _moleculeJs.HinderedRotorPotential).s_useSineTerms);
    let buttonTextContentSelected = (0, _moleculeJs.HinderedRotorPotential).s_useSineTerms + (0, _appJs.sy_selected);
    let buttonTextContentDeselected = (0, _moleculeJs.HinderedRotorPotential).s_useSineTerms + (0, _appJs.sy_deselected);
    let button = (0, _htmlJs.createButton)(buttonTextContentDeselected, undefined, margin);
...
function processMoleculeList(xml, molecules) {
    // Create div to contain the molecules list.
    let mlDiv = (0, _htmlJs.createDiv)(undefined, (0, _appJs.boundary1));
    // Get the XML "moleculeList" element.
    let xml_ml = (0, _xmlJs.getSingularElement)(xml, (0, _mesmerJs.MoleculeList).tagName);
    // Check the XML "moleculeList" element has one or more "molecule" elements and no other elements.
    let mlTagNames = new Set();
    xml_ml.childNodes.forEach(function(node) {
        mlTagNames.add(node.nodeName);
    ...
    if (!mlTagNames.has((0, _moleculeJs.Molecule).tagName)) {
        console.warn('Expecting tags with "' + (0, _moleculeJs.Molecule).tagName + '" tagName but there are none!');
        return mlDiv;
    }
    // Process the XML "molecule" elements.
    let xml_ms = xml_ml.getElementsByTagName((0, _moleculeJs.Molecule).tagName);
    let xml_msl = xml_ms.length;
    console.log("Number of molecules=" + xml_msl);
    //xml_molecules.forEach(function (xml_molecule) { // Cannot iterate over HTMLCollectionOf<Element> like this.
    for(let i = 0; i < xml_msl; i++){
        // Create a new Molecule.
        let mDivID = (0, _appJs.addRID)((0, _moleculeJs.Molecule).tagName, i);
        let mDiv = (0, _htmlJs.createDiv)(mDivID);
        let attributes = (0, _xmlJs.getAttributes)(xml_ms[i]);
        let m = new (0, _moleculeJs.Molecule)(attributes, attributes.get((0, _moleculeJs.Molecule).s_id));
        (0, _appJs.addMolecule)(m, molecules);
        // Create collapsible Molecule HTMLDivElement.
        let mcDivID = (0, _appJs.addRID)(mDivID, (0, _appJs.s_container));
        ...
        // Add edit Name button.
        addEditIDButton(m, mcDiv.querySelector((0, _htmlJs.s_button)), mDiv, (0, _appJs.level1));
        // Description
        mDiv.appendChild(processDescription((0, _appJs.addRID)(mDivID, (0, _appJs.s_description)), m.getDescription.bind(m), m.setDescription.bind(m), (0, _appJs.boundary1), (0, _appJs.level1)));
        // Init metadataList.
        //console.log("Init metadataList.");
        let xml_mls = xml_ms[i].getElementsByTagName((0, _metadataJs.MetadataList).tagName);
        if (xml_mls.length > 0) {
            if (xml_mls.length > 1) console.warn("Expecting 1 or 0 " + (0, _metadataJs.MetadataList).tagName + " but finding " + xml_mls.length + ". Loading the first of these...");
            // Create collapsible MetadataList HTMLDivElement.
            let mlDivID = (0, _appJs.addRID)(mDivID, (0, _metadataJs.MetadataList).tagName);
            let mlDiv = (0, _htmlJs.createDiv)(mlDivID);
            let mlcDivID = (0, _appJs.addRID)(mlDivID, (0, _appJs.s_container));
            let mlcDiv = (0, _htmlJs.getCollapsibleDiv)(mlcDivID, mDiv, null, mlDiv, (0, _metadataJs.MetadataList).tagName, (0, _appJs.boundary1), (0, _appJs.level1));
            ...
            for(let j = 0; j < xml_ms.length; j++){
                ...
                mlDiv.appendChild(addMetadata(m, md, ml, (0, _appJs.addRID)(mlDivID, j), (0, _appJs.boundary1), (0, _appJs.level1)));
            }
            moleculeTagNames.delete((0, _metadataJs.MetadataList).tagName);
        }
        // Init atoms.
        let xml_aas = xml_ms[i].getElementsByTagName((0, _moleculeJs.AtomArray).tagName);
        // Create collapsible AtomArray HTMLDivElement.
        let aaDivID = (0, _appJs.addRID)(mDivID, (0, _moleculeJs.AtomArray).tagName);
        let aaDiv = (0, _htmlJs.createDiv)(aaDivID);
        let aacDivID = (0, _appJs.addRID)(aaDivID, (0, _appJs.s_container));
        let aacDiv = (0, _htmlJs.getCollapsibleDiv)(aacDivID, mDiv, null, aaDiv, (0, _moleculeJs.AtomArray).tagName, (0, _appJs.boundary1), (0, _appJs.level1));
        ...
        let aa = new (0, _moleculeJs.AtomArray)(new Map());
        m.setAtoms(aa);
        ...
        for(let j = 0; j < xml_as.length; j++)aaDiv.appendChild(addAtom(m, aaDivID, aa, new (0, _moleculeJs.Atom)((0, _xmlJs.getAttributes)(xml_as[j]), m), (0, _appJs.boundary1), (0, _appJs.level1)));
        aaDiv.appendChild(getAddAtomButton(m, aaDiv, (0, _moleculeJs.Atom).tagName, (0, _appJs.boundary1), (0, _appJs.level1)));
        moleculeTagNames.delete((0, _moleculeJs.Atom).tagName);
        // Init bonds.
        let xml_bas = xml_ms[i].getElementsByTagName((0, _moleculeJs.BondArray).tagName);
        // Create collapsible BondArray HTMLDivElement.
        let baDivID = (0, _appJs.addRID)(mDivID, (0, _moleculeJs.BondArray).tagName);
        let baDiv = (0, _htmlJs.createDiv)(baDivID);
        let bacDivID = (0, _appJs.addRID)(baDivID, (0, _appJs.s_container));
        let bacDiv = (0, _htmlJs.getCollapsibleDiv)(bacDivID, mDiv, null, baDiv, (0, _moleculeJs.BondArray).tagName, (0, _appJs.boundary1), (0, _appJs.level1));
        ...
        let ba = new (0, _moleculeJs.BondArray)(new Map());
        m.setBonds(ba);
        ...
        let xml_bs = xml_ms[i].getElementsByTagName((0, _moleculeJs.Bond).tagName);
        for(let j = 0; j < xml_bs.length; j++){
            ...
            if (b_attributes.has((0, _moleculeJs.Bond).s_id)) baDiv.appendChild(addBond(m, baDivID, m.getAtoms().atoms, ba, new (0, _moleculeJs.Bond)((0, _xmlJs.getAttributes)(xml_bs[j]), m), (0, _appJs.boundary1), (0, _appJs.level1)));
        }
        ...
        let viewerDivID = (0, _appJs.addRID)(mDivID, "viewer");
        let viewerDiv = (0, _htmlJs.createDiv)(viewerDivID);
        let viewercDivID = (0, _appJs.addRID)(viewerDivID, (0, _appJs.s_container));
        let viewercDiv = (0, _htmlJs.getCollapsibleDiv)(viewercDivID, mDiv, null, viewerDiv, "viewer", (0, _appJs.boundary1), (0, _appJs.level1));
        create3DViewer(m, viewerDiv, (0, _appJs.boundary1), (0, _appJs.level1));
        // Init properties.
        let xml_pls = xml_ms[i].getElementsByTagName((0, _moleculeJs.PropertyList).tagName);
        // Create a new collapsible div for the PropertyList.
        let plDivID = (0, _appJs.addRID)(mDivID, (0, _moleculeJs.PropertyList).tagName);
        let plDiv = (0, _htmlJs.createDiv)(plDivID);
        let plcDivID = (0, _appJs.addRID)(plDivID, (0, _appJs.s_container));
        let plcDiv = (0, _htmlJs.getCollapsibleDiv)(plcDivID, mDiv, null, plDiv, (0, _moleculeJs.PropertyList).tagName, (0, _appJs.boundary1), (0, _appJs.level1));
        ...
        let pl = new (0, _moleculeJs.PropertyList)(new Map());
        m.setPropertyList(pl);
        ...
        let pap = new Set((0, _moleculeJs.PropertyArray).propertyDictRefs);
        let xml_ps = xml_ms[i].getElementsByTagName((0, _moleculeJs.Property).tagName);
        ...
        */ // Organise EnergyTransferModel.
        ...
        // Organise DOSCMethod.
        let xml_dms = xml_ms[i].getElementsByTagName((0, _moleculeJs.DOSCMethod).tagName);
        if (xml_dms.length > 0) {
            ...
            mDiv.appendChild((0, _htmlJs.createLabelWithSelect)((0, _moleculeJs.DOSCMethod).tagName, (0, _moleculeJs.DOSCMethod).xsi_typeOptions, (0, _moleculeJs.DOSCMethod).tagName, doscm.getXsiType(), (0, _appJs.addRID)(mDivID, (0, _moleculeJs.DOSCMethod).tagName), (0, _appJs.boundary1), (0, _appJs.level1)));
        ...
        if (xml_dcms.length > 0) {
            ...
            m.setDistributionCalcMethod(dcm);
            let dcmDivID = (0, _appJs.addRID)(mDivID, (0, _moleculeJs.DistributionCalcMethod).tagName);
            let dcmDiv = (0, _htmlJs.createDiv)(dcmDivID);
            mDiv.appendChild(dcmDiv);
            // Create label.
            dcmDiv.appendChild((0, _htmlJs.createLabel)((0, _moleculeJs.DistributionCalcMethod).tagName + " " + (0, _utilJs.mapToString)(dcmAttributes), (0, _appJs.level1)));
        ...
        if (xml_dosl.length > 0) {
            ...
            let doslDivID = (0, _appJs.addRID)(mDivID, (0, _moleculeJs.DensityOfStatesList).tagName);
            let doslDiv = (0, _htmlJs.createDiv)(doslDivID);
            let doslcDivID = (0, _appJs.addRID)(doslDivID, (0, _appJs.s_container));
            let doslcDiv = (0, _htmlJs.getCollapsibleDiv)(doslcDivID, mDiv, null, doslDiv, (0, _moleculeJs.DensityOfStatesList).tagName, (0, _appJs.boundary1), (0, _appJs.level1));
            ...
            if (xml_dos.length == 0) throw new Error("Expecting 1 or more " + (0, _moleculeJs.DensityOfStates).tagName + " but finding 0!");
                ...
                let t = (0, _htmlJs.createTable)((0, _appJs.addRID)(doslDivID, (0, _appJs.s_table)), (0, _appJs.level1));
                ...
                for(let j = 0; j < xml_dos.length; j++){
                    ...
                    dosl.addDensityOfStates(dos);
                    let dosDivID = (0, _appJs.addRID)(doslDivID, j);
                ...
                (0, _appJs.addSaveAsCSVButton)(dosl.toCSV, doslDiv, t, m.getID() + "_" + (0, _moleculeJs.DensityOfStatesList).tagName, (0, _appJs.level1));
            }
        ...
        if (xml_tts.length > 0) {
            ...
            let tt = new (0, _moleculeJs.ThermoTable)((0, _xmlJs.getAttributes)(xml_tts[0]));
            // Create collapsible div.
            let ttDivId = (0, _appJs.addRID)(mDivID, (0, _moleculeJs.ThermoTable).tagName);
            let ttDiv = (0, _htmlJs.createDiv)(ttDivId);
            let ttcDivId = (0, _appJs.addRID)(ttDivId, (0, _appJs.s_container));
            let ttcDiv = (0, _htmlJs.getCollapsibleDiv)(ttcDivId, mDiv, null, ttDiv, tttn, (0, _appJs.boundary1), (0, _appJs.level1));
            let tvs;
            let tvtn = (0, _moleculeJs.ThermoValue).tagName;
            ...
            if (xml_tvs.length == 0) throw new Error("Expecting 1 or more " + tvtn + " but finding 0!");
                ...
                tvs = [];
                let t = (0, _htmlJs.createTable)((0, _appJs.addRID)(ttDivId, (0, _appJs.s_table)), (0, _appJs.level1));
                (0, _htmlJs.addTableRow)(t, tt.getHeader());
                for(let j = 0; j < xml_tvs.length; j++){
                    let tv = new (0, _moleculeJs.ThermoValue)((0, _xmlJs.getAttributes)(xml_tvs[j]));
                    tvs.push(tv);
                ...
                ttDiv.appendChild(t);
                tt.init(tvs);
                (0, _appJs.addSaveAsCSVButton)(tt.toCSV.bind(tt), ttDiv, t, m.getID() + "_" + (0, _moleculeJs.ThermoTable).tagName, (0, _appJs.level1));
            }
            m.setThermoTable(tt);
        ...
        let xml_edms = xml_ms[i].getElementsByTagName((0, _moleculeJs.ExtraDOSCMethod).tagName);
        if (xml_edms.length > 0) for(let j = 0; j < xml_edms.length; j++){
            let edm = new (0, _moleculeJs.ExtraDOSCMethod)((0, _xmlJs.getAttributes)(xml_edms[j]));
            // Create collapsible ExtraDOSCMethod HTMLDivElement.
            let edmDivID = (0, _appJs.addRID)(mDivID, (0, _moleculeJs.ExtraDOSCMethod).tagName, j);
            let edmDiv = (0, _htmlJs.createDiv)(edmDivID);
            let edmcDivID = (0, _appJs.addRID)(edmDivID, (0, _appJs.s_container));
            let edmcDiv = (0, _htmlJs.getCollapsibleDiv)(edmcDivID, mDiv, null, edmDiv, (0, _moleculeJs.ExtraDOSCMethod).tagName, (0, _appJs.boundary1), (0, _appJs.level1));
            // Read bondRef.
            ...
            if (xml_brs.length > 0) {
                ...
                let lws = (0, _htmlJs.createLabelWithSelect)((0, _moleculeJs.BondRef).tagName, bids, (0, _moleculeJs.BondRef).tagName, br.value, (0, _appJs.addRID)(edmDivID, (0, _moleculeJs.BondRef).tagName), (0, _appJs.boundary1), (0, _appJs.level1));
            ...
            if (xml_hrps.length > 0) {
                ...
                let hrp = new (0, _moleculeJs.HinderedRotorPotential)(hrpAttributes);
                // Create collapsible HinderedRotorPotential HTMLDivElement.
                let hrpDivID = (0, _appJs.addRID)(edmDivID, (0, _moleculeJs.HinderedRotorPotential).tagName);
                let hrpDiv = (0, _htmlJs.createDiv)(hrpDivID);
                let hrpcDivID = (0, _appJs.addRID)(hrpDivID, (0, _appJs.s_container));
                let hrpcDiv = (0, _htmlJs.getCollapsibleDiv)(hrpcDivID, edmDiv, null, hrpDiv, (0, _moleculeJs.HinderedRotorPotential).tagName, (0, _appJs.boundary1), (0, _appJs.level1));
                // Format.
                let lws = (0, _htmlJs.createLabelWithSelect)((0, _moleculeJs.HinderedRotorPotential).s_format, (0, _moleculeJs.HinderedRotorPotential).formats, (0, _moleculeJs.HinderedRotorPotential).tagName, hrp.getFormat(), (0, _appJs.addRID)(hrpDivID, (0, _moleculeJs.HinderedRotorPotential).s_format), (0, _appJs.boundary1), (0, _appJs.level1));
                hrpDiv.appendChild(lws);
                // Units.
                (0, _appJs.addAnyUnits)((0, _mesmerJs.Mesmer).energyUnits, hrpAttributes, hrpDiv, lws, (0, _appJs.addRID)(hrpDivID, (0, _moleculeJs.HinderedRotorPotential).s_units), (0, _moleculeJs.HinderedRotorPotential).tagName, (0, _appJs.boundary1), (0, _appJs.level1));
                // ExpansionSize.
                let es = hrp.getExpansionSize() ?? (0, _appJs.s_undefined);
                hrpDiv.appendChild((0, _htmlJs.createLabelWithInput)("text", (0, _appJs.addRID)(hrpDivID, (0, _moleculeJs.HinderedRotorPotential).s_expansionSize), (0, _appJs.boundary1), (0, _appJs.level1), (event)=>{
                    let target = event.target;
                    ...
                    (0, _htmlJs.resizeInputElement)(target);
                }, es, (0, _moleculeJs.HinderedRotorPotential).s_expansionSize));
                // Add useSineTerms.
                processUseSineTerms(hrpDiv, hrp, (0, _appJs.level1));
                // Load PotentialPoints.
                // Create collapsible HinderedRotorPotential PotentialPoint HTMLDivElement.
                let ppsDivID = (0, _appJs.addRID)(hrpDivID, (0, _moleculeJs.PotentialPoint).tagName);
                let ppsDiv = (0, _htmlJs.createDiv)(ppsDivID);
                let ppscDivID = (0, _appJs.addRID)(ppsDivID, (0, _appJs.s_container));
                ...
                let pps = [];
                let xml_pps = xml_hrps[0].getElementsByTagName((0, _moleculeJs.PotentialPoint).tagName);
                for(let k = 0; k < xml_pps.length; k++){
                    let pp = new (0, _moleculeJs.PotentialPoint)((0, _xmlJs.getAttributes)(xml_pps[k]));
                    pps.push(pp);
                    let ppDivID = (0, _appJs.addRID)(ppsDivID, k);
                    let ppDiv = (0, _htmlJs.createFlexDiv)(ppDivID, (0, _appJs.level1));
                    ppsDiv.appendChild(ppDiv);
                    let l = (0, _htmlJs.createLabel)((0, _moleculeJs.PotentialPoint).tagName + " " + k, (0, _appJs.boundary1));
                    ppDiv.appendChild(l);
                    // Process angle
                    let a = pp.getAngle() ?? (0, _appJs.s_undefined);
                    let anglelwi = (0, _htmlJs.createLabelWithInput)("text", (0, _appJs.addRID)(ppDivID, (0, _moleculeJs.PotentialPoint).s_angle), (0, _appJs.boundary1), (0, _appJs.boundary1), (event)=>{
                        let target = event.target;
                        // Check the input is a number.
                        ...
                        (0, _htmlJs.resizeInputElement)(target);
                    }, a, (0, _moleculeJs.PotentialPoint).s_angle);
                    ppDiv.appendChild(anglelwi);
                    // Create a new div element for the potential.
                    let potentialLabel = (0, _htmlJs.createLabel)((0, _moleculeJs.PotentialPoint).s_potential, (0, _appJs.boundary1));
                    ppDiv.appendChild(potentialLabel);
                    let potentialInputElementId = (0, _appJs.addRID)(ppDivID, (0, _moleculeJs.PotentialPoint).s_potential);
            ...
            // Read periodicities.
            let xml_periodicities = xml_edms[j].getElementsByTagName((0, _moleculeJs.Periodicity).tagName);
            if (xml_periodicities.length > 0) {
                ...
                let periodicity = new (0, _moleculeJs.Periodicity)((0, _xmlJs.getAttributes)(xml_periodicities[0]), new (0, _bigJsDefault.default)(valueString));
                edm.setPeriodicity(periodicity);
                let lwi = (0, _htmlJs.createLabelWithInput)("text", (0, _appJs.addRID)(edmDivID, (0, _moleculeJs.Periodicity).tagName), (0, _appJs.boundary1), (0, _appJs.level1), (event)=>{
                    let target = event.target;
        ...
        let xml_ReservoirSize = xml_ms[i].getElementsByTagName((0, _moleculeJs.ReservoirSize).tagName);
        if (xml_ReservoirSize.length > 0) {
            ...
            let inputDiv = (0, _htmlJs.createLabelWithInput)("number", m.getID() + "_" + (0, _moleculeJs.ReservoirSize).tagName, (0, _appJs.boundary1), (0, _appJs.level1), (event)=>{
                let target = event.target;
    ...
    // Create an add molecule button.
    let mb = getAddMoleculeButton(mlDiv, molecules);
    // Create add from library button.
    let lb = getAddFromLibraryButton(mlDiv, mb, molecules);
    return mlDiv;
}
/**
 * @param pl The PropertyList.
 ...
 */ function createProperty(pap, pl, xml, plDiv, molecule, boundary, level) {
    ...
    if (p.dictRef == (0, _moleculeJs.ZPE).dictRef) // "me:ZPE", scalar, Mesmer.energyUnits.
    (0, _appJs.processPropertyScalarNumber)(pl, p, (0, _mesmerJs.Mesmer).energyUnits, molecule, xml, plDiv, boundary, level);
    else if (p.dictRef == (0, _moleculeJs.Hf0).dictRef) // "me:Hf0", scalar, Mesmer.energyUnits.
    (0, _appJs.processPropertyScalarNumber)(pl, p, (0, _mesmerJs.Mesmer).energyUnits, molecule, xml, plDiv, boundary, level);
    else if (p.dictRef == (0, _moleculeJs.HfAT0).dictRef) // "me:HfAT0", scalar, Mesmer.energyUnits.
    (0, _appJs.processPropertyScalarNumber)(pl, p, (0, _mesmerJs.Mesmer).energyUnits, molecule, xml, plDiv, boundary, level);
    else if (p.dictRef == (0, _moleculeJs.Hf298).dictRef) // "me:Hf298", scalar, Mesmer.energyUnits.
    (0, _appJs.processPropertyScalarNumber)(pl, p, (0, _mesmerJs.Mesmer).energyUnits, molecule, xml, plDiv, boundary, level);
    else if (p.dictRef == (0, _moleculeJs.RotConsts).dictRef) // "me:rotConsts", array, Mesmer.frequencyUnits.
    (0, _appJs.processPropertyScalarNumber)(pl, p, (0, _mesmerJs.Mesmer).frequencyUnits, molecule, xml, plDiv, boundary, level);
    else if (p.dictRef == (0, _moleculeJs.SymmetryNumber).dictRef) // "me:symmetryNumber", scalar, No units.
    (0, _appJs.processPropertyScalarNumber)(pl, p, undefined, molecule, xml, plDiv, boundary, level);
    else if (p.dictRef == (0, _moleculeJs.TSOpticalSymmetryNumber).dictRef) // "me:TSOpticalSymmetryNumber", scalar, No units.
    (0, _appJs.processPropertyScalarNumber)(pl, p, undefined, molecule, xml, plDiv, boundary, level);
    else if (p.dictRef == (0, _moleculeJs.FrequenciesScaleFactor).dictRef) // "me:frequenciesScaleFactor", scalar, No units.
    (0, _appJs.processPropertyScalarNumber)(pl, p, undefined, molecule, xml, plDiv, boundary, level);
    else if (p.dictRef == (0, _moleculeJs.VibFreqs).dictRef) // "me:vibFreqs", array, cm-1.
    (0, _appJs.processPropertyScalarNumber)(pl, p, undefined, molecule, xml, plDiv, boundary, level);
    else if (p.dictRef == (0, _moleculeJs.MW).dictRef) // "me:MW", scalar, amu.
    (0, _appJs.processPropertyScalarNumber)(pl, p, undefined, molecule, xml, plDiv, boundary, level);
    else if (p.dictRef == (0, _moleculeJs.SpinMultiplicity).dictRef) // "me:spinMultiplicity", scalar, No units.
    (0, _appJs.processPropertyScalarNumber)(pl, p, undefined, molecule, xml, plDiv, boundary, level);
    else if (p.dictRef == (0, _moleculeJs.Epsilon).dictRef) // "me:epsilon", scalar, K (fixed).
    (0, _appJs.processPropertyScalarNumber)(pl, p, undefined, molecule, xml, plDiv, boundary, level);
    else if (p.dictRef == (0, _moleculeJs.Sigma).dictRef) // "me:sigma", scalar, Å (fixed).
    (0, _appJs.processPropertyScalarNumber)(pl, p, undefined, molecule, xml, plDiv, boundary, level);
    else if (p.dictRef == (0, _moleculeJs.Hessian).dictRef) // "me:hessian", matrix, kJ/mol/Å2 or kcal/mol/Å2 or Hartree/Å2.
    (0, _appJs.processPropertyScalarNumber)(pl, p, undefined, molecule, xml, plDiv, boundary, level);
    else if (p.dictRef == (0, _moleculeJs.EinsteinAij).dictRef) // "me:EinsteinAij", array, s-1 (fixed).
    (0, _appJs.processPropertyScalarNumber)(pl, p, undefined, molecule, xml, plDiv, boundary, level);
    else if (p.dictRef == (0, _moleculeJs.EinsteinBij).dictRef) // "me:EinsteinBij", array, m3/J/s2 (fixed).
    (0, _appJs.processPropertyScalarNumber)(pl, p, undefined, molecule, xml, plDiv, boundary, level);
 ...
 */ function processEnergyTransferModel(etm, molecule, element, moleculeDiv) {
    ...
    if (xml_deltaEDowns.length > 0) {
        ...
        let etmdivID = (0, _appJs.addRID)(moleculeDiv.id, (0, _moleculeJs.EnergyTransferModel).tagName);
        let etmDiv = document.createElement("div");
        let etmcDivID = (0, _appJs.addRID)(etmdivID, (0, _appJs.s_container));
        let etmcDiv = (0, _htmlJs.getCollapsibleDiv)(etmcDivID, moleculeDiv, null, etmDiv, (0, _moleculeJs.EnergyTransferModel).tagName, (0, _appJs.boundary1), (0, _appJs.level1));
        let deltaEDowns = [];
        for(let k = 0; k < xml_deltaEDowns.length; k++){
            ...
            // Create a new div element for the input.
            let id = (0, _appJs.addRID)(etmdivID, (0, _moleculeJs.DeltaEDown).tagName, k);
...
function create3DViewer(molecule, moleculeDiv, boundary, level) {
    ...
    let viewerContainerDivID = (0, _appJs.addRID)(moleculeDiv.id, "viewerContainer");
    let viewerContainerDiv = (0, _htmlJs.createDiv)(viewerContainerDivID, level);
    moleculeDiv.appendChild(viewerContainerDiv);
    let viewerDivID = (0, _appJs.addRID)(moleculeDiv.id, "viewer");
    ...
    showAtomLabels, showBondLabels) {
        ...
        molecule.getAtoms().atoms.forEach(function(atom) {
            ...
            else color = (0, _mesmerJs.Mesmer).atomColors.get(et) || "Purple";
            //let am: number = Mesmer.atomMasses.get(atom.getElementType()) || 1;
            let radius;
            if (et == undefined) radius = 100;
            else radius = (0, _mesmerJs.Mesmer).atomRadii.get(atom.getElementType()) || 100;
    ...
    let atomLabelbutton = createLabelButton(s_Atom_Labels, (0, _appJs.addRID)(viewerDivID, s_Atom_Labels), showAtomLabels, (newState)=>showAtomLabels = newState);
    viewerContainerDiv.appendChild(atomLabelbutton);
    // Bond Labels.
    let s_Bond_Labels = "Bond Labels";
    let bondLabelbutton = createLabelButton(s_Bond_Labels, (0, _appJs.addRID)(viewerDivID, s_Bond_Labels), showBondLabels, (newState)=>showBondLabels = newState);
    viewerContainerDiv.appendChild(bondLabelbutton);
    // Add a save button to save the viewer as an image.
    let saveButton = (0, _htmlJs.createButton)("Save as PNG", (0, _appJs.addRID)(viewerDivID, (0, _appJs.s_save)), (0, _appJs.boundary1));
    saveButton.addEventListener("click", ()=>{
...
},{"big.js":"91nMZ","./app.js":"dPB9w","./conditions.js":"aksKl","./html.js":"aLPSL","./mesmer.js":"kMp4Q","./metadata.js":"aKNnu","./molecule.js":"ahQNx","./util.js":"f0Rnl","./xml.js":"7znDa","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"iXoY9":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
/**
...
function getAddReactionButton(rlDiv, reactions, molecules) {
    let rb = (0, _htmlJs.createButton)((0, _appJs.s_Add_sy_add), (0, _appJs.addRID)((0, _reactionJs.Reaction).tagName, (0, _htmlJs.s_button)), (0, _appJs.level1));
    rlDiv.appendChild(rb);
    rb.addEventListener("click", ()=>{
        let reactionAttributes = new Map();
        reactionAttributes.set((0, _reactionJs.Reaction).s_id, "R" + reactions.size);
        let r = new (0, _reactionJs.Reaction)(reactionAttributes);
        reactions.set(r.id, r);
        let rDivID = (0, _appJs.addRID)((0, _reactionJs.Reaction).tagName, r.id);
        ...
        let selectReactant = (0, _htmlJs.createSelectElement)((0, _appJs.getMoleculeKeys)(molecules), "select", "", (0, _appJs.addRID)(rcDivID, (0, _reactionJs.Reactant).tagName, (0, _htmlJs.s_select)), (0, _appJs.level1));
...
function processReactionList(xml, reactions) {
    ...
    let xml_reactionList = (0, _xmlJs.getSingularElement)(xml, (0, _mesmer.ReactionList).tagName);
    ...
    for(let i = 0; i < xml_reactions.length; i++){
        ...
        let reactionDivID = (0, _appJs.addRID)((0, _reactionJs.Reaction).tagName, i);
        ...
        if (xml_reactants.length > 0) {
            ...
            let rsDivID = (0, _appJs.addRID)(reactionDivID, (0, _reactionJs.Reactant).tagName);
            ...
            for(let j = 0; j < xml_reactants.length; j++){
                let reactantDivID = (0, _appJs.addRID)(rsDivID, (0, _reactionJs.Reactant).tagName, j);
                ...
                let lws = (0, _htmlJs.createLabelWithSelect)(molecule.getRef() + " role", (0, _reactionJs.Reactant).roleOptions, "Role", molecule.getRole(), (0, _appJs.addRID)(reactantDivID, (0, _htmlJs.s_select)), (0, _appJs.boundary1), (0, _appJs.level1));
        ...
        if (xml_products.length > 0) {
            ...
            let psDivID = (0, _appJs.addRID)(reactionDivID, (0, _reactionJs.Product).tagName);
            ...
            for(let j = 0; j < xml_products.length; j++){
                ...
                let lws = (0, _htmlJs.createLabelWithSelect)(molecule.getRef() + " role", (0, _reactionJs.Product).roleOptions, molecule.getRole(), molecule.getRef(), (0, _appJs.addRID)(psDivID, j, "Role"), (0, _appJs.boundary1), (0, _appJs.level1));
                let select = lws.querySelector("select");
        ...
        if (xml_tunneling.length > 0) {
            ...
            let lws = (0, _htmlJs.createLabelWithSelect)((0, _reactionJs.Tunneling).tagName, (0, _reactionJs.Tunneling).options, "Tunneling", tunneling.getName(), (0, _appJs.addRID)(reactionDivID, (0, _reactionJs.Tunneling).tagName), (0, _appJs.boundary1), (0, _appJs.level1));
            lws.querySelector("select")?.addEventListener("change", (event)=>{
                let target = event.target;
        ...
        if (xml_transitionStates.length > 0) {
            // Create collapsible div.
            let tsDivID = (0, _appJs.addRID)(reactionDivID, (0, _reactionJs.TransitionState).tagName);
        ...
        if (xml_MCRCMethod.length > 0) {
            if (xml_MCRCMethod.length > 1) throw new Error("Expecting 1 " + (0, _reactionJs.MCRCMethod).tagName + " but finding " + xml_MCRCMethod.length + "!");
                ...
                let mmDivId = (0, _appJs.addRID)(reactionDivID, (0, _reactionJs.MCRCMethod).tagName);
                ...
                if (type == (0, _reactionJs.MesmerILT).xsiType || type == (0, _reactionJs.MesmerILT).xsiType2) {
                    ...
                    let mmcDiv = (0, _htmlJs.getCollapsibleDiv)(mmcDivId, reactionDiv, null, mmDiv, (0, _reactionJs.MCRCMethod).tagName, (0, _appJs.boundary1), (0, _appJs.level1));
                    ...
                    if (xml_pe != null) {
                        if (xml_pe[0] != null) {
                            ...
                            // Create a new div element for the input.
                            let lwi = (0, _htmlJs.createLabelWithInput)("number", (0, _appJs.addRID)(mmDivId, (0, _reactionJs.PreExponential).tagName, (0, _appJs.s_input)), (0, _appJs.boundary1), (0, _appJs.level1), (event)=>{
                                let target = event.target;
                            ...
                            (0, _appJs.addAnyUnits)(undefined, peAttributes, lwi, null, (0, _appJs.addRID)(mmDivId, (0, _reactionJs.PreExponential).tagName), (0, _reactionJs.PreExponential).tagName, (0, _appJs.boundary1), (0, _appJs.boundary1));
                            mmDiv.appendChild(lwi);
                        }
                    }
                    ...
                    if (xml_ae != null) {
                        if (xml_ae[0] != null) {
                            ...
                            // Create a new div element for the input.
                            let lwi = (0, _htmlJs.createLabelWithInput)("number", (0, _appJs.addRID)(mmDivId, (0, _reactionJs.ActivationEnergy).tagName, (0, _appJs.s_input)), (0, _appJs.boundary1), (0, _appJs.level1), (event)=>{
                                let target = event.target;
                            ...
                            (0, _appJs.addAnyUnits)(undefined, aeAttributes, lwi, null, (0, _appJs.addRID)(mmDivId, (0, _reactionJs.ActivationEnergy).tagName), (0, _reactionJs.ActivationEnergy).tagName, (0, _appJs.boundary1), (0, _appJs.boundary1));
                            mmDiv.appendChild(lwi);
                        }
                    }
                    ...
                    if (xml_ti != null) {
                        if (xml_ti[0] != null) {
                            ...
                            mm.setTInfinity(ti);
                            // Create a new div element for the input.
                            let lwi = (0, _htmlJs.createLabelWithInput)("number", (0, _appJs.addRID)(mmDivId, (0, _reactionJs.TInfinity).tagName, (0, _appJs.s_input)), (0, _appJs.boundary1), (0, _appJs.level1), (event)=>{
                                let target = event.target;
                            ...
                            (0, _appJs.addAnyUnits)(undefined, tiAttributes, lwi, null, (0, _appJs.addRID)(mmDivId, (0, _reactionJs.TInfinity).tagName), (0, _reactionJs.TInfinity).tagName, (0, _appJs.boundary1), (0, _appJs.boundary1));
                            mmDiv.appendChild(lwi);
                        }
                    }
                    ...
                    if (xml_ni != null) {
                        if (xml_ni[0] != null) {
                            ...
                            mm.setNInfinity(ni);
                            // Create a new div element for the input.
                            let lwi = (0, _htmlJs.createLabelWithInput)("number", (0, _appJs.addRID)(mmDivId, (0, _reactionJs.NInfinity).tagName, (0, _appJs.s_input)), (0, _appJs.boundary1), (0, _appJs.level1), (event)=>{
                                let target = event.target;
                            ...
                            (0, _appJs.addAnyUnits)(undefined, niAttributes, lwi, null, (0, _appJs.addRID)(mmDivId, (0, _reactionJs.NInfinity).tagName), (0, _reactionJs.NInfinity).tagName, (0, _appJs.boundary1), (0, _appJs.boundary1));
                            mmDiv.appendChild(lwi);
                        }
                    }
                } else {
        ...
        if (xml_erc.length > 0) {
            ...
            let id = (0, _appJs.addRID)(reactionDivID, (0, _reactionJs.ExcessReactantConc).tagName);
        ...
        if (xml_crl.length > 0) {
            ...
            let crlDivID = (0, _appJs.addRID)(reactionDivID, (0, _reactionJs.CanonicalRateList).tagName);
            let crlDiv = (0, _htmlJs.createDiv)(crlDivID);
            let crlcDivID = (0, _appJs.addRID)(crlDivID, (0, _appJs.s_container));
            let crlcDiv = (0, _htmlJs.getCollapsibleDiv)(crlcDivID, reactionDiv, null, crlDiv, (0, _reactionJs.CanonicalRateList).tagName, (0, _appJs.boundary1), (0, _appJs.level1));
            reactionDiv.appendChild(crlcDiv);
            //let id = addID(reaction.id, CanonicalRateList.tagName);
            // me:description.
            let xml_d = xml_crl[0].getElementsByTagName((0, _mesmer.Description).tagName);
            //console.log("xml_d.length=" + xml_d.length);
            if (xml_d.length > 0) {
                if (xml_d.length > 1) throw new Error("Expecting 1 " + (0, _mesmer.Description).tagName + " but finding " + xml_d.length + "!");
                let description = (0, _xmlJs.getNodeValue)((0, _xmlJs.getFirstChildNode)(xml_d[0]));
                //console.log("description=" + description);
                crl.setDescription(new (0, _mesmer.Description)((0, _xmlJs.getAttributes)(xml_d[0]), description));
            ...
            if (xml_k.length > 0) {
                ...
                let t = (0, _htmlJs.createTable)((0, _appJs.addRID)(crlDivID, (0, _reactionJs.Kinf).tagName, (0, _appJs.s_table)), (0, _appJs.level1));
                crlDiv.appendChild(t);
                for(let j = 0; j < xml_k.length; j++){
                    let k = new (0, _reactionJs.Kinf)((0, _xmlJs.getAttributes)(xml_k[j]));
                    crl.addKinf(k);
                    // T.
                    let xml_T = xml_k[j].getElementsByTagName((0, _mesmer.T).tagName);
                    ...
                    if (xml_T.length > 0) {
                        ...
                        k.setT(new (0, _mesmer.T)((0, _xmlJs.getAttributes)(xml_T[0]), value));
                    }
                    // Val.
                ...
                (0, _appJs.addSaveAsCSVButton)(crl.toCSV.bind(crl), crlDiv, t, reaction.id + "_" + (0, _reactionJs.CanonicalRateList).tagName, (0, _appJs.level1));
            }
        }
    }
...
},{"big.js":"91nMZ","./app.js":"dPB9w","./html.js":"aLPSL","./mesmer":"kMp4Q","./molecule.js":"ahQNx","./reaction.js":"8grVN","./util.js":"f0Rnl","./xml.js":"7znDa","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"8grVN":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
/**
...
},{"big.js":"91nMZ","./xml.js":"7znDa","./mesmer.js":"kMp4Q","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"i75Ii":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
/**
...
function processConditions(xml, conditionsIDs, molecules) {
    console.log((0, _conditions.Conditions).tagName);
    ...
    let xml_conditionss = xml.getElementsByTagName((0, _conditions.Conditions).tagName);
    for(let i = 0; i < xml_conditionss.length; i++){
        let xml_conditions = xml_conditionss[i];
        // Create a collapsible div for each conditions.
        let cDivID = (0, _app.addRID)((0, _conditions.Conditions).tagName, i.toString());
        let cDiv = (0, _html.createDiv)(cDivID, (0, _app.boundary1));
        let ccDivID = (0, _app.addRID)(cDivID, (0, _app.s_container));
        let ccDiv = (0, _html.getCollapsibleDiv)(ccDivID, conditionssDiv, null, cDiv, (0, _conditions.Conditions).tagName + " " + i.toString(), (0, _app.boundary1), (0, _app.level1));
 ...
 */ function handleBathGases(conditions, cDiv, xml_conditions, conditionsIDs, molecules) {
    // Bath Gases
    // Create a collapsible div.
    let bsDivID = conditionsIDs.addID(cDiv.id, (0, _conditions.BathGas).tagName);
    let bsDiv = (0, _html.createDiv)(bsDivID);
    let bscDivID = conditionsIDs.addID(cDiv.id, (0, _conditions.BathGas).tagName, (0, _app.s_container));
    let bscDiv = (0, _html.getCollapsibleDiv)(bscDivID, cDiv, null, bsDiv, (0, _conditions.BathGas).tagName, (0, _app.boundary1), (0, _app.level1));
    // Add add button.
    let addBathGasButton = (0, _html.createButton)((0, _app.s_Add_sy_add), conditionsIDs.addID(cDiv.id, (0, _conditions.BathGas).tagName, (0, _html.s_button)), (0, _app.level1));
    bsDiv.appendChild(addBathGasButton);
    addBathGasButton.addEventListener("click", ()=>{
        let bathGas = new (0, _conditions.BathGas)(new Map(), (0, _app.s_selectOption));
        let bathGasIndex = conditions.addBathGas(bathGas);
        let div = (0, _html.createFlexDiv)(undefined, (0, _app.level1));
        let id = conditionsIDs.addID(cDiv.id, (0, _conditions.BathGas).tagName, bathGasIndex.toString());
        let select = createSelectElementBathGas(Array.from((0, _app.getMoleculeKeys)(molecules)), bathGas, true, id);
        select.classList.add((0, _conditions.BathGas).tagName);
        div.appendChild(select);
    ...
    if (xml_conditions != null) {
        let xml_bathGases = Array.from(xml_conditions.children).filter((child)=>child.tagName === (0, _conditions.BathGas).tagName);
        if (xml_bathGases.length > 0) for(let i = 0; i < xml_bathGases.length; i++){
            ...
            let bathGas = new (0, _conditions.BathGas)(attributes, moleculeID);
            ...
            let div = (0, _html.createFlexDiv)(undefined, (0, _app.level1));
            let id = conditionsIDs.addID(cDiv.id, (0, _conditions.BathGas).tagName, 0);
 ...
 */ function handlePTs(conditions, cDiv, xml_conditions, conditionsIDs, molecules) {
    ...
    // Create collapsible div.
    let pTsDivId = conditionsIDs.addID(cDiv.id, (0, _conditions.PTs).tagName);
    let pTsDiv = (0, _html.createDiv)(pTsDivId);
    let pTscDivId = conditionsIDs.addID(cDiv.id, pTsDivId, (0, _app.s_container));
    let pTscDiv = (0, _html.getCollapsibleDiv)(pTscDivId, cDiv, null, pTsDiv, (0, _conditions.PTs).tagName, (0, _app.boundary1), (0, _app.level1));
    let pTs;
    if (xml_conditions) {
        let xml_PTss = xml_conditions.getElementsByTagName((0, _conditions.PTs).tagName);
        if (xml_PTss.length > 0) {
            if (xml_PTss.length > 1) throw new Error("Expecting 1 " + (0, _conditions.PTs).tagName + " but finding " + xml_PTss.length + "!");
            let attributes = (0, _xml.getAttributes)(xml_PTss[0]);
            let xml_PTpairs = xml_PTss[0].getElementsByTagName((0, _conditions.PTpair).tagName);
            if (xml_PTpairs.length == 0) throw new Error("Expecting 1 or more " + (0, _conditions.PTpair).tagName + " but finding 0!");
            else {
                pTs = new (0, _conditions.PTs)(attributes);
                for(let i = 0; i < xml_PTpairs.length; i++){
                    let pTpairAttributes = (0, _xml.getAttributes)(xml_PTpairs[i]);
                    //console.log("pTpairAttributes=" + mapToString(pTpairAttributes));
                    let pTpair = new (0, _conditions.PTpair)(pTpairAttributes);
                    pTs.add(pTpair);
                    // BathGas.
                    let xml_bathGass = xml_PTpairs[i].getElementsByTagName((0, _conditions.BathGas).tagName);
                    if (xml_bathGass.length > 0) {
                        if (xml_bathGass.length > 1) console.warn("xml_bathGass.length=" + xml_bathGass.length);
                        pTpair.setBathGas(new (0, _conditions.BathGas)((0, _xml.getAttributes)(xml_bathGass[0]), (0, _xml.getNodeValue)((0, _xml.getFirstChildNode)(xml_bathGass[0]))));
                    }
                    // ExperimentRate.
                    let xml_ers = xml_PTpairs[i].getElementsByTagName((0, _conditions.ExperimentalRate).tagName);
                    if (xml_ers.length > 0) {
                        if (xml_ers.length > 1) console.warn("xml_experimentRates.length=" + xml_ers.length);
                        pTpair.setExperimentalRate(new (0, _conditions.ExperimentalRate)((0, _xml.getAttributes)(xml_ers[0]), new (0, _bigJsDefault.default)((0, _xml.getNodeValue)((0, _xml.getFirstChildNode)(xml_ers[0])).trim())));
                    }
                    // ExperimentalYield.
                    let xml_eys = xml_PTpairs[i].getElementsByTagName((0, _conditions.ExperimentalYield).tagName);
                    if (xml_eys.length > 0) {
                        if (xml_eys.length > 1) console.warn("xml_experimentalYields.length=" + xml_eys.length);
                        pTpair.setExperimentalYield(new (0, _conditions.ExperimentalYield)((0, _xml.getAttributes)(xml_eys[0]), new (0, _bigJsDefault.default)((0, _xml.getNodeValue)((0, _xml.getFirstChildNode)(xml_eys[0])).trim())));
                    }
                    // ExperimentalEigenvalue.
                    let xml_ees = xml_PTpairs[i].getElementsByTagName((0, _conditions.ExperimentalEigenvalue).tagName);
                    if (xml_ees.length > 0) {
                        if (xml_ees.length > 1) console.warn("xml_experimentalEigenvalues.length=" + xml_ees.length);
                        pTpair.setExperimentalEigenvalue(new (0, _conditions.ExperimentalEigenvalue)((0, _xml.getAttributes)(xml_ees[0]), new (0, _bigJsDefault.default)((0, _xml.getNodeValue)((0, _xml.getFirstChildNode)(xml_ees[0])).trim())));
                    }
                    // Create pTpairDiv.
                    pTsDiv.appendChild(createPTpairDiv(pTs, pTsDiv, pTpair, cDiv.id, i, moleculeKeys, (0, _app.level1)));
                }
            }
        } else pTs = new (0, _conditions.PTs)(new Map());
    } else pTs = new (0, _conditions.PTs)(new Map());
    ...
    addMultipleButton.addEventListener("click", ()=>{
        ...
        let div = (0, _html.createFlexDiv)(undefined, (0, _app.level1));
        let addFromSpreadsheetId = (0, _app.addRID)((0, _conditions.PTs).tagName, "addFromSpreadsheet");
        ...
        input.addEventListener("change", ()=>{
            ...
            if (input.value.length > 0) {
                ...
                for(let i = 1; i < pTpairsArray.length; i++){
                    ...
                    pTpair.setP(p);
                    pTpair.setT(t);
                    if (index.has((0, _conditions.PTpair).s_excessReactantConc)) {
                        let excessReactantConIndex = index.get((0, _conditions.PTpair).s_excessReactantConc);
                        let excessReactantConc = pTpairArray[excessReactantConIndex];
                        pTpairAttributes.set((0, _conditions.PTpair).s_excessReactantConc, excessReactantConc);
                    }
                    if (index.has((0, _conditions.PTpair).s_percentExcessReactantConc)) {
                        let percentExcessReactantConIndex = index.get((0, _conditions.PTpair).s_percentExcessReactantConc);
                        let percentExcessReactantConc = pTpairArray[percentExcessReactantConIndex];
                        pTpairAttributes.set((0, _conditions.PTpair).s_percentExcessReactantConc, percentExcessReactantConc);
                    }
                    if (index.has((0, _conditions.PTpair).s_precision)) {
                        console.log("index.has(PTpair.s_precision)");
                        let precisionIndex = index.get((0, _conditions.PTpair).s_precision);
                        let precision = pTpairArray[precisionIndex];
                        pTpairAttributes.set((0, _conditions.PTpair).s_precision, precision);
                    //console.log("precision=" + precision);
                    }
                    if (index.has((0, _conditions.BathGas).tagName)) {
                        let bathGasIndex = index.get((0, _conditions.BathGas).tagName);
                        let bathGas = pTpairArray[bathGasIndex];
                        pTpair.setBathGas(new (0, _conditions.BathGas)(new Map(), bathGas));
                    }
                    if (index.has((0, _conditions.ExperimentalRate).tagName)) {
                        let eri = index.get((0, _conditions.ExperimentalRate).tagName);
                        let er = pTpairArray[eri];
                        if (er.length > 0) {
                            pTpairAttributes.set((0, _conditions.ExperimentalRate).tagName, er);
                            pTpair.setExperimentalRate(new (0, _conditions.ExperimentalRate)(new Map(), new (0, _bigJsDefault.default)(er)));
                            // Set the attributes of the experimentalRate.
                            // ref1.
                            let err1i = index.get((0, _conditions.ExperimentalRate).tagName + "_" + (0, _conditions.ExperimentalRate).s_ref1);
                            let err1 = pTpairArray[err1i];
                            pTpair.getExperimentalRate()?.setRef1(err1);
                            // ref2.
                            let err2i = index.get((0, _conditions.ExperimentalRate).tagName + "_" + (0, _conditions.ExperimentalRate).s_ref2);
                            let err2 = pTpairArray[err2i];
                            pTpair.getExperimentalRate()?.setRef2(err2);
                            // refReaction.
                            let errri = index.get((0, _conditions.ExperimentalRate).tagName + "_" + (0, _conditions.ExperimentalRate).s_refReaction);
                            let errr = pTpairArray[errri];
                            pTpair.getExperimentalRate()?.setRefReaction(errr);
                            // error.
                            let erei = index.get((0, _conditions.ExperimentalRate).tagName + "_" + (0, _conditions.ExperimentalRate).s_error);
                            let ere = pTpairArray[erei];
                            pTpair.getExperimentalRate()?.setError(new (0, _bigJsDefault.default)(ere));
                        }
                    }
                    if (index.has((0, _conditions.ExperimentalYield).tagName)) {
                        let eyi = index.get((0, _conditions.ExperimentalYield).tagName);
                        let ey = pTpairArray[eyi];
                        if (ey.length > 0) {
                            pTpair.setExperimentalYield(new (0, _conditions.ExperimentalYield)(new Map(), new (0, _bigJsDefault.default)(ey)));
                            // Set the attributes of the experimentalYield.
                            // ref.
                            let eyri = index.get((0, _conditions.ExperimentalYield).tagName + "_" + (0, _conditions.ExperimentalYield).s_ref);
                            let eyr = pTpairArray[eyri];
                            pTpair.getExperimentalYield()?.setRef(eyr);
                            // yieldTime.
                            let eyyti = index.get((0, _conditions.ExperimentalYield).tagName + "_" + (0, _conditions.ExperimentalYield).s_yieldTime);
                            let eyyt = pTpairArray[eyyti];
                            pTpair.getExperimentalYield()?.setYieldTime(new (0, _bigJsDefault.default)(eyyt));
                            // error.
                            let eyei = index.get((0, _conditions.ExperimentalYield).tagName + "_" + (0, _conditions.ExperimentalYield).s_error);
                            let eye = pTpairArray[eyei];
                            pTpair.getExperimentalYield()?.setError(new (0, _bigJsDefault.default)(eye));
                        }
                    }
                    if (index.has((0, _conditions.ExperimentalEigenvalue).tagName)) {
                        let eei = index.get((0, _conditions.ExperimentalEigenvalue).tagName);
                        let ee = pTpairArray[eei];
                        if (ee.length > 0) {
                            pTpair.setExperimentalEigenvalue(new (0, _conditions.ExperimentalEigenvalue)(new Map(), new (0, _bigJsDefault.default)(ee)));
                            // Set the attributes of the experimentalEigenvalue.
                            // EigenvalueID.
                            let eeeidi = index.get((0, _conditions.ExperimentalEigenvalue).tagName + "_" + (0, _conditions.ExperimentalEigenvalue).s_EigenvalueID);
                            let eeeid = pTpairArray[eeeidi];
                            pTpair.getExperimentalEigenvalue()?.setEigenvalueID(eeeid);
                            // error.
                            let eeei = index.get((0, _conditions.ExperimentalEigenvalue).tagName + "_" + (0, _conditions.ExperimentalEigenvalue).s_error);
 ...
 * @param level The level.
 * @returns A button.
 */ function createAddConditionsButton(conditionssDiv, conditionsIDs, molecules) {
    ...
    button.addEventListener("click", (event)=>{
        ...
        if (i > 0) {
            let aboveElement = document.getElementById((0, _util.getID)((0, _conditions.Conditions).tagName, (i - 1).toString(), (0, _app.s_container)));
        ...
        let ccDiv = (0, _html.getCollapsibleDiv)(ccDivID, conditionssDiv, elementToInsertBefore, cDiv, (0, _conditions.Conditions).tagName + " " + i.toString(), (0, _app.boundary1), (0, _app.level1));
        // Add the conditions
 ...
 */ function addConditions(attributes, i) {
    let conditions = new (0, _conditions.Conditions)(attributes, i);
 ...
 */ function createPTpairDiv(pTs, pTsDiv, pTpair, cDivID, pTIndex, moleculeKeys, level) {
    ...
    addPorT(pTpairDiv, (0, _conditions.PTpair).s_P, pTpair.getP.bind(pTpair), pTpair.setP.bind(pTpair)); // The binding is necessary to maintain the correct "this".
    (0, _app.addAnyUnits)((0, _mesmer.Mesmer).pressureUnits, pTpair.attributes, pTpairDiv, null, (0, _conditions.PTpair).tagName, (0, _conditions.PTpair).tagName, (0, _app.boundary1), (0, _app.level1));
    addPorT(pTpairDiv, (0, _conditions.PTpair).s_T, pTpair.getT.bind(pTpair), pTpair.setT.bind(pTpair)); // The binding is necessary to maintain the correct "this".
    ...
    addAttribute(pTpairDiv, pTpair, (0, _conditions.PTpair).s_excessReactantConc, createExcessReactantConcInputElement);
    ...
    addAttribute(pTpairDiv, pTpair, (0, _conditions.PTpair).s_precision, createPrecisionSelectElement);
    ...
    */ // ExperimentalRate.
    ...
    addExperimentalElement(pTpairDiv, pTpair, pTIndex, (0, _conditions.ExperimentalRate).tagName, (pTpair)=>pTpair.getExperimentalRate(), createExperimentalRateDetails);
    // ExperimentalYield.
    ...
    addExperimentalElement(pTpairDiv, pTpair, pTIndex, (0, _conditions.ExperimentalYield).tagName, (pTpair)=>pTpair.getExperimentalYield(), createExperimentalYieldDetails);
    // ExperimentalEigenvalue.
    ...
    addExperimentalElement(pTpairDiv, pTpair, pTIndex, (0, _conditions.ExperimentalEigenvalue).tagName, (pTpair)=>pTpair.getExperimentalEigenvalue(), createExperimentalEigenvalueDetails);
 ...
 */ function addPorT(pTpairDiv, name, getter, setter) {
    let lwi = (0, _html.createLabelWithInput)("text", (0, _conditions.PTpair).tagName + "_" + name, (0, _app.boundary1), (0, _app.level0), (event)=>{
        let target = event.target;
        try {
            setter(new (0, _bigJsDefault.default)(target.value));
 ...
 */ function addPercentExcessReactantConc(pTpairDiv, pTpair) {
    let id = (0, _app.addRID)(pTpairDiv.id, (0, _conditions.PTpair).s_percentExcessReactantConc);
    let div = (0, _html.createDiv)(id, (0, _app.boundary1));
    pTpairDiv.appendChild(div);
    let attribute = (0, _conditions.PTpair).s_percentExcessReactantConc;
 ...
 */ function addBathGas(pTpairDiv, pTpair, moleculeKeys) {
    let id = (0, _app.addRID)(pTpairDiv.id, (0, _conditions.BathGas).tagName);
    let div = (0, _html.createDiv)(id, (0, _app.boundary1));
    pTpairDiv.appendChild(div);
    let tagName = (0, _conditions.BathGas).tagName;
 ...
 */ function createPrecisionSelectElement(id, pTpair) {
    let value;
    if (pTpair.attributes.has((0, _conditions.PTpair).s_precision)) value = pTpair.attributes.get((0, _conditions.PTpair).s_precision);
    else value = (0, _mesmer.Mesmer).precisionOptions[0];
    let select = (0, _html.createSelectElement)((0, _mesmer.Mesmer).precisionOptions, (0, _conditions.PTpair).s_precision, value, id, (0, _app.boundary1));
    select.addEventListener("change", (event)=>{
        let target = event.target;
        pTpair.setPrecision(target.value);
        console.log("Set " + (0, _conditions.PTpair).s_precision + " to " + target.value);
 ...
 */ function createExcessReactantConcInputElement(id, pTpair) {
    ...
    let value;
    if (pTpair.attributes.has((0, _conditions.PTpair).s_excessReactantConc)) value = pTpair.attributes.get((0, _conditions.PTpair).s_excessReactantConc);
    else value = NaN.toString();
    console.log((0, _conditions.PTpair).s_excessReactantConc + "=" + value);
    input.value = value;
    input.addEventListener("change", (event)=>{
        let target = event.target;
        pTpair.setExcessReactantConc(target.value);
        console.log("Set " + (0, _conditions.PTpair).s_excessReactantConc + " to " + target.value);
        (0, _html.resizeInputElement)(target);
    });
    (0, _html.resizeInputElement)(input);
    return input;
 ...
 */ function createBathGasSelectElement(id, pTpair, bathGas, first, moleculeKeys) {
    ...
    select.addEventListener("change", (event)=>{
        let target = event.target;
        pTpair.setBathGas(new (0, _conditions.BathGas)(new Map(), target.value));
 ...
 */ function createSelectElementBathGas(options, bathGas, first, id) {
    ...
    if (bathGas == undefined) {
        bathGas = new (0, _conditions.BathGas)(new Map(), (0, _app.s_selectOption));
        value = (0, _app.s_selectOption);
    } else value = bathGas.value;
    let select = (0, _html.createSelectElement)(options, (0, _conditions.BathGas).tagName, value, (0, _app.addRID)(id, (0, _html.s_select)), (0, _app.boundary1));
    select.classList.add((0, _conditions.BathGas).tagName);
    ...
    select.addEventListener("change", (event)=>{
        ...
        console.log("Added " + target.value + " as " + (0, _conditions.BathGas).tagName);
        (0, _html.resizeSelectElement)(target);
    });
    select.value = value;
    (0, _html.resizeSelectElement)(select);
...
function createExperimentalRateDetails(id, pTpair) {
    return addExperimentalDetails(pTpair, id, (pTpair)=>pTpair.getExperimentalRate(), (pTpair, value)=>pTpair.setExperimentalRate(value), (0, _conditions.ExperimentalRate), [
        {
            tagName: (0, _conditions.ExperimentalRate).tagName,
            type: "number",
        ...
        {
            tagName: (0, _conditions.ExperimentalRate).tagName + "_" + (0, _conditions.ExperimentalRate).s_ref1,
            type: "text",
            eventHandler: (event, target)=>pTpair.getExperimentalRate()?.setRef1(target.value),
            valueGetter: ()=>pTpair.getExperimentalRate().getRef1()
        },
        {
            tagName: (0, _conditions.ExperimentalRate).tagName + "_" + (0, _conditions.ExperimentalRate).s_ref2,
            type: "text",
            eventHandler: (event, target)=>pTpair.getExperimentalRate()?.setRef2(target.value),
            valueGetter: ()=>pTpair.getExperimentalRate().getRef2()
        },
        {
            tagName: (0, _conditions.ExperimentalRate).tagName + "_" + (0, _conditions.ExperimentalRate).s_refReaction,
            type: "text",
        ...
        {
            tagName: (0, _conditions.ExperimentalRate).tagName + "_" + (0, _conditions.ExperimentalRate).s_error,
            type: "number",
...
function createExperimentalYieldDetails(id, pTpair) {
    return addExperimentalDetails(pTpair, id, (pTpair)=>pTpair.getExperimentalYield(), (pTpair, value)=>pTpair.setExperimentalYield(value), (0, _conditions.ExperimentalYield), [
        {
            tagName: (0, _conditions.ExperimentalYield).tagName,
            type: "number",
        ...
        {
            tagName: (0, _conditions.ExperimentalYield).tagName + "_" + (0, _conditions.ExperimentalYield).s_ref,
            type: "text",
            eventHandler: (event, target)=>pTpair.getExperimentalYield()?.setRef(target.value),
            valueGetter: ()=>pTpair.getExperimentalYield().getRef()
        },
        {
            tagName: (0, _conditions.ExperimentalYield).tagName + "_" + (0, _conditions.ExperimentalYield).s_yieldTime,
            type: "number",
        ...
        {
            tagName: (0, _conditions.ExperimentalYield).tagName + "_" + (0, _conditions.ExperimentalYield).s_error,
            type: "number",
...
function createExperimentalEigenvalueDetails(id, pTpair) {
    return addExperimentalDetails(pTpair, id, (pTpair)=>pTpair.getExperimentalEigenvalue(), (pTpair, value)=>pTpair.setExperimentalEigenvalue(value), (0, _conditions.ExperimentalEigenvalue), [
        {
            tagName: (0, _conditions.ExperimentalEigenvalue).tagName,
            type: "number",
        ...
        {
            tagName: (0, _conditions.ExperimentalEigenvalue).tagName + "_" + (0, _conditions.ExperimentalEigenvalue).s_EigenvalueID,
            type: "text",
        ...
        {
            tagName: (0, _conditions.ExperimentalEigenvalue).tagName + "_" + (0, _conditions.ExperimentalEigenvalue).s_error,
            type: "number",
...
},{"big.js":"91nMZ","./app":"dPB9w","./conditions":"aksKl","./html":"aLPSL","./mesmer":"kMp4Q","./util":"f0Rnl","./xml":"7znDa","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"cN8Dd":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
/**
...
var _html = require("./html");
var _mesmer = require("./mesmer");
var _modelParameters = require("./modelParameters");
var _util = require("./util");
var _xml = require("./xml");
function processModelParameters(xml, modelParametersIDs) {
    console.log((0, _modelParameters.ModelParameters).tagName);
    // Create a div for the modelParameterss.
    let mpsDiv = (0, _html.createDiv)(undefined, (0, _app.boundary1));
    let xml_mps = xml.getElementsByTagName((0, _modelParameters.ModelParameters).tagName);
    for(let i = 0; i < xml_mps.length; i++){
        // Create a collapsible div for the model parameters.
        let mpDivID = (0, _app.addRID)((0, _modelParameters.ModelParameters).tagName, i.toString());
        let mpDiv = (0, _html.createDiv)(mpDivID, (0, _app.boundary1));
        let mpcDivID = (0, _app.addRID)(mpDivID, (0, _app.s_container));
        let mpcDiv = (0, _html.getCollapsibleDiv)(mpcDivID, mpsDiv, null, mpDiv, (0, _modelParameters.ModelParameters).tagName + " " + i.toString(), (0, _app.boundary1), (0, _app.level1));
        ...
        processModelParametersN(mp, modelParametersIDs, xml_mps[i], mpDiv, (0, _control.AutomaticallySetMaxEne), mp.setAutomaticallySetMaxEne, mp.removeAutomaticallySetMaxEne.bind(mp));
        processModelParametersN(mp, modelParametersIDs, xml_mps[i], mpDiv, (0, _modelParameters.EnergyAboveTheTopHill), mp.setEnergyAboveTheTopHill, mp.removeEnergyAboveTheTopHill.bind(mp));
        processModelParametersN(mp, modelParametersIDs, xml_mps[i], mpDiv, (0, _modelParameters.MaxTemperature), mp.setMaxTemperature, mp.removeMaxTemperature.bind(mp));
 ...
 */ function addModelParameters(attributes, i) {
    let mp = new (0, _modelParameters.ModelParameters)(attributes, i);
 ...
 */ function processGrainSize(mps, xml_mps, mpsDiv, modelParametersIDs) {
    let tagName = (0, _modelParameters.GrainSize).tagName;
    ...
    if (xml_mps != null) {
        ...
        if (xml.length == 1) {
            ...
            let value = new (0, _bigJsDefault.default)(valueString);
            gs = new (0, _modelParameters.GrainSize)((0, _xml.getAttributes)(xml[0]), value);
            button.textContent = buttonTextContentSelected;
            createInputModelParameters(mps, div, gs, idi, valueString, mps.setGrainSize, (0, _mesmer.Mesmer).energyUnits);
            button.classList.toggle((0, _app.s_optionOff));
        } else {
    ...
    button.addEventListener("click", ()=>{
        ...
        if (!mps.index.has((0, _modelParameters.GrainSize).tagName)) {
            createInputModelParameters(mps, div, gs, idi, valueString, mps.setGrainSize, (0, _mesmer.Mesmer).energyUnits);
            button.textContent = buttonTextContentSelected;
        } else {
            mps.removeGrainSize();
 ...
 */ function addGrainSize(mps, mpsDiv, modelParametersIDs) {
    ...
    button.addEventListener("click", ()=>{
        ...
        if (!mps.index.has((0, _modelParameters.GrainSize).tagName)) {
            ...
            gs = getDefaultGrainsize(tagName);
            mps.setGrainSize(gs);
            createInputModelParameters(mps, div, gs, idi, gs.value.toString(), mps.setGrainSize, (0, _mesmer.Mesmer).energyUnits);
            button.textContent = buttonTextContentSelected;
        } else {
...
function getDefaultGrainsize(tagName) {
    ...
    return new (0, _modelParameters.GrainSize)(attributes, value);
}
/**
 ...
 * @param level The level.
 * @returns A button.
 */ function createAddModelParametersButton(mpsDiv, modelParametersIDs) {
    let button = (0, _html.createButton)((0, _app.s_Add_sy_add), undefined, (0, _app.level1));
    let tn = (0, _modelParameters.ModelParameters).tagName;
    ...
    button.addEventListener("click", (event)=>{
        ...
        processModelParametersN(mp, modelParametersIDs, null, mpDiv, (0, _control.AutomaticallySetMaxEne), mp.setAutomaticallySetMaxEne, mp.removeAutomaticallySetMaxEne);
        processModelParametersN(mp, modelParametersIDs, null, mpDiv, (0, _modelParameters.EnergyAboveTheTopHill), mp.setEnergyAboveTheTopHill, mp.removeEnergyAboveTheTopHill);
        processModelParametersN(mp, modelParametersIDs, null, mpDiv, (0, _modelParameters.MaxTemperature), mp.setMaxTemperature, mp.removeMaxTemperature);
        // Add a remove modelParameters button.
        let removeButton = (0, _app.addRemoveButton)(mpDiv, (0, _app.level1), (0, _app.mesmer).removeModelParameters.bind((0, _app.mesmer)), i);
        removeButton.addEventListener("click", (event)=>{
            // Remove the modelParameters.
...
},{"big.js":"91nMZ","./app":"dPB9w","./control":"Qx5gu","./html":"aLPSL","./mesmer":"kMp4Q","./modelParameters":"kQHfz","./util":"f0Rnl","./xml":"7znDa","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"9Xsct":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
/**
...
var _util = require("./util");
var _xml = require("./xml");
function processControl(xml, controlIDs) {
    console.log((0, _control.Control).tagName);
    // Create a div for the controls.
    let controlsDiv = (0, _html.createDiv)(undefined, (0, _app.boundary1));
    // Get the XML "me:control" element.
    let xml_controls = xml.getElementsByTagName((0, _control.Control).tagName);
    for(let i = 0; i < xml_controls.length; i++){
        //console.log("Control " + i);
        let xml_control = xml_controls[i];
        // Create a collapsible divfor the control.
        let cDivID = (0, _app.addRID)((0, _control.Control).tagName, i.toString());
        let cDiv = (0, _html.createDiv)(cDivID, (0, _app.boundary1));
        controlsDiv.appendChild(cDiv);
        let ccDivID = (0, _app.addRID)(cDivID, (0, _app.s_container));
        let ccDiv = (0, _html.getCollapsibleDiv)(ccDivID, controlsDiv, null, cDiv, (0, _control.Control).tagName + " " + i.toString(), (0, _app.boundary1), (0, _app.level1));
        ...
        handleTestMicroRates(control, cDiv, controlIDs, null, (0, _app.level1));
        handleCalcMethod(control, cDiv, controlIDs, xml_control, (0, _app.level1));
        getControlItems(control).forEach((item)=>{
            handleControl(control, cDiv, controlIDs, onOffControls, xml_control, (0, _app.level1), item.class, item.setMethod, item.removeMethod, true);
        });
        // me:ForceMacroDetailedBalance
        let xml_fdb = xml_control.getElementsByTagName((0, _control.ForceMacroDetailedBalance).tagName);
        if (xml_fdb.length == 1) {
            ...
            let fdb = new (0, _control.ForceMacroDetailedBalance)(fdb_attributes, s);
            control.setForceMacroDetailedBalance(fdb);
            let fdbDiv = (0, _html.createFlexDiv)(controlIDs.addID(cDivID, (0, _control.ForceMacroDetailedBalance).tagName), (0, _app.level1));
            cDiv.appendChild(fdbDiv);
            let fdbl = (0, _html.createLabel)((0, _control.ForceMacroDetailedBalance).tagName + " " + (0, _util.mapToString)(fdb_attributes) + " " + s, (0, _app.boundary1));
    ...
    createAddControlButton(controlsDiv, controlIDs, (0, _app.level1));
    return controlsDiv;
}
/**
 * @param control The control.
 ...
 */ function getControlOptionsSimple(control) {
    return [
        ...
        {
            class: (0, _control.ForceMacroDetailedBalance),
 ...
 * @param level The level.
 * @returns A button.
 */ function createAddControlButton(controlsDiv, controlIDs, level) {
    let button = (0, _html.createButton)((0, _app.s_Add_sy_add), undefined, (0, _app.level1));
    ...
    button.addEventListener("click", (event)=>{
        ...
        let elementToInsertBefore;
        if (i > 0) {
            let aboveElement = document.getElementById((0, _util.getID)((0, _control.Control).tagName, (i - 1).toString(), (0, _app.s_container)));
            let nextElementSibling = aboveElement.nextElementSibling;
        ...
        // Create a collapsible div for each conditions.
        let ccDivID = (0, _app.addRID)(cDivID, (0, _app.s_container));
        let ccDiv = (0, _html.getCollapsibleDiv)(ccDivID, controlsDiv, elementToInsertBefore, cDiv, (0, _control.Control).tagName + " " + i.toString(), (0, _app.boundary1), (0, _app.level1));
        // Add the control
        ...
        // Create a div for the on/off controls.
        let onOffControlsDiv = (0, _html.createFlexDiv)(undefined, level);
        let orderedOnOffControls = new Map([
            ...onOffControls.entries()
        ].sort());
        orderedOnOffControls.forEach((button)=>{
 ...
 */ function addControl(attributes, i) {
    let control = new (0, _control.Control)(attributes, i);
 ...
 * @param xml_control The xml control. 
 * @param level The level.
 */ function handleCalcMethod(control, cDiv, controlIDs, xml_control, level) {
    //console.log("handleCalcMethod " + (xml_control == null));
    let div = (0, _html.createFlexDiv)(undefined, level);
    cDiv.appendChild(div);
    let tagName = (0, _control.CalcMethod).tagName;
    let buttonTextContentSelected = tagName + (0, _app.sy_selected);
    let buttonTextContentDeselected = tagName + (0, _app.sy_deselected);
    let button = (0, _html.createButton)(buttonTextContentDeselected, undefined, (0, _app.boundary1));
    div.appendChild(button);
    ...
    let options = (0, _control.CalcMethod).options;
    ...
    button.addEventListener("click", (event)=>{
        ...
        if (!control.index.has(tagName)) {
            ...
            // Remove select.
            //remove(divCmId);
            (0, _app.remove)(divCmDetailsId);
            (0, _app.remove)(divCmDetailsSelectId);
 ...
 * @param xml_control The xml control.
 * @param level The level.
 */ function handleTestMicroRates(control, cDiv, controlIDs, xml_control, level) {
    let tagName = (0, _control.TestMicroRates).tagName;
    let divID = controlIDs.addID(cDiv.id, tagName);
    let div = (0, _html.createFlexDiv)(divID, level);
    cDiv.appendChild(div);
    let buttonTextContentSelected = tagName + (0, _app.sy_selected);
    let buttonTextContentDeselected = tagName + (0, _app.sy_deselected);
    let button = (0, _html.createButton)(tagName, controlIDs.addID(cDiv.id, tagName, (0, _html.s_button)), (0, _app.boundary1));
    div.appendChild(button);
    button.classList.add((0, _app.s_optionOn));
    button.classList.add((0, _app.s_optionOff));
    let idTmax = controlIDs.addID(cDiv.id, tagName, (0, _control.Tmax).tagName);
    let idTmin = controlIDs.addID(cDiv.id, tagName, (0, _control.Tmin).tagName);
    let idTstep = controlIDs.addID(cDiv.id, tagName, (0, _control.Tstep).tagName);
    if (xml_control) {
        let xml = xml_control.getElementsByTagName(tagName);
        if (xml.length == 1) {
 ...
 */ function createTestMicroRates(control, div, xml_tmr, idTmax, idTmin, idTstep) {
    ...
    if (xml_tmr != null && xml_tmr.length > 0) {
        ...
        tmr = new (0, _control.TestMicroRates)(attributes);
    } else {
        attributes = new Map();
 ...
 */ function getCalcMethod(control, divCm, xml, options, attributes, tagName, xsi_type, divCmDetailsId, divCmDetailsSelectId) {
    ...
    if (xsi_type == (0, _control.CalcMethodSimpleCalc).xsi_type || xsi_type == (0, _control.CalcMethodSimpleCalc).xsi_type2) //console.log("CalcMethodSimpleCalc");
    cm = new (0, _control.CalcMethodSimpleCalc)(attributes);
    else if (xsi_type == (0, _control.CalcMethodGridSearch).xsi_type || xsi_type == (0, _control.CalcMethodGridSearch).xsi_type2) cm = new (0, _control.CalcMethodGridSearch)(attributes);
    else if (xsi_type == (0, _control.CalcMethodFitting).xsi_type || xsi_type == (0, _control.CalcMethodFitting).xsi_type2) {
        let cmf = new (0, _control.CalcMethodFitting)(attributes);
        cm = cmf;
        // FittingIterations.
        let fi_xml = xml[0].getElementsByTagName((0, _control.FittingIterations).tagName);
        if (fi_xml.length > 0) {
            if (fi_xml.length == 1) {
                let value = new (0, _bigJsDefault.default)((0, _xml.getNodeValue)((0, _xml.getFirstChildNode)(fi_xml[0])));
                let fittingIterations = new (0, _control.FittingIterations)((0, _xml.getAttributes)(fi_xml[0]), value);
    ...
    } else if (xsi_type == (0, _control.CalcMethodMarquardt).xsi_type || xsi_type == (0, _control.CalcMethodMarquardt).xsi_type2) {
        let cmm = new (0, _control.CalcMethodMarquardt)(attributes);
        cm = cmm;
        function processElement(xml, ClassConstructor, setterMethod) {
            let tagName = (0, _control.MarquardtIterations).tagName;
        ...
        processElement(xml, (0, _control.MarquardtIterations), cmm.setMarquardtIterations.bind(cmm));
        processElement(xml, (0, _control.MarquardtTolerance), cmm.setMarquardtTolerance.bind(cmm));
        processElement(xml, (0, _control.MarquardtDerivDelta), cmm.setMarquardtDerivDelta.bind(cmm));
        processCalcMethodMarquardt(divCmDetails, cmm);
    } else if (xsi_type == (0, _control.CalcMethodAnalyticalRepresentation).xsi_type || xsi_type == (0, _control.CalcMethodAnalyticalRepresentation).xsi_type2) {
        let cmar = new (0, _control.CalcMethodAnalyticalRepresentation)(attributes);
        cm = cmar;
        ...
        processElement(xml, (0, _control.Format), cmar.setFormat.bind(cmar), false);
        processElement(xml, (0, _control.Precision), cmar.setPrecision.bind(cmar), false);
        processElement(xml, (0, _control.ChebNumTemp), cmar.setChebNumTemp.bind(cmar), true);
        processElement(xml, (0, _control.ChebNumConc), cmar.setChebNumConc.bind(cmar), true);
        processElement(xml, (0, _control.ChebMaxTemp), cmar.setChebMaxTemp.bind(cmar), true);
        processElement(xml, (0, _control.ChebMinTemp), cmar.setChebMinTemp.bind(cmar), true);
        processElement(xml, (0, _control.ChebMaxConc), cmar.setChebMaxConc.bind(cmar), true);
        processElement(xml, (0, _control.ChebMinConc), cmar.setChebMinConc.bind(cmar), true);
        processElement(xml, (0, _control.ChebTExSize), cmar.setChebTExSize.bind(cmar), true);
        processElement(xml, (0, _control.ChebPExSize), cmar.setChebPExSize.bind(cmar), true);
        processCalcMethodAnalyticalRepresentation(divCmDetails, cmar);
    } else if (xsi_type == (0, _control.CalcMethodThermodynamicTable).xsi_type || xsi_type == (0, _control.CalcMethodThermodynamicTable).xsi_type2) {
        let cmtt = new (0, _control.CalcMethodThermodynamicTable)(attributes);
        cm = cmtt;
        function processElement(xml, ClassConstructor, setterMethod) {
            let tagName = ClassConstructor.tagName;
            let elementXml = xml[0].getElementsByTagName(tagName);
        ...
        processElement(xml, (0, _control.Tmin), cmtt.setTmin.bind(cmtt));
        processElement(xml, (0, _control.Tmid), cmtt.setTmid.bind(cmtt));
        processElement(xml, (0, _control.Tmax), cmtt.setTmax.bind(cmtt));
        processElement(xml, (0, _control.Tstep), cmtt.setTstep.bind(cmtt));
        processCalcMethodThermodynamicTable(divCmDetails, cmtt);
    } else if (xsi_type == (0, _control.CalcMethodSensitivityAnalysis).xsi_type || xsi_type == (0, _control.CalcMethodSensitivityAnalysis).xsi_type2) {
        let cmsa = new (0, _control.CalcMethodSensitivityAnalysis)(attributes);
        cm = cmsa;
        function processElement(xml, ClassConstructor, setterMethod) {
            let tagName = ClassConstructor.tagName;
            let elementXml = xml[0].getElementsByTagName(tagName);
        ...
        processElement(xml, (0, _control.SensitivityAnalysisSamples), cmsa.setSensitivityAnalysisSamples.bind(cmsa));
        processElement(xml, (0, _control.SensitivityAnalysisOrder), cmsa.setSensitivityAnalysisOrder.bind(cmsa));
        processElement(xml, (0, _control.SensitivityNumVarRedIters), cmsa.setSensitivityNumVarRedIters.bind(cmsa));
        processElement(xml, (0, _control.SensitivityVarRedMethod), cmsa.setSensitivityVarRedMethod.bind(cmsa));
 ...
 */ function processCalcMethodFitting(divCmDetails, cm) {
    // FittingIterations.
    let fi = cm.getFittingIterations() || new (0, _control.FittingIterations)(new Map(), (0, _app.big0));
    cm.setFittingIterations(fi);
    divCmDetails.appendChild((0, _html.createLabelWithInput)("number", (0, _util.getID)(divCmDetails.id, (0, _control.FittingIterations).tagName, (0, _app.s_input)), (0, _app.boundary1), (0, _app.level0), (event)=>{
        let target = event.target;
        // Check the value is a number.
        if ((0, _util.isNumeric)(target.value)) {
        ...
        (0, _html.resizeInputElement)(target);
    }, fi.value.toString(), (0, _control.FittingIterations).tagName));
}
/**
 * @param divCmDetails The details div.
 ...
 */ function processCalcMethodMarquardt(divCmDetails, cm) {
    ...
    // MarquardtIterations.
    let mi = cm.getMarquardtIterations() || new (0, _control.MarquardtIterations)(new Map(), (0, _app.big0));
    cm.setMarquardtIterations(mi);
    createLabelWithInputForObject(mi, divCmDetails, (0, _app.boundary1), (0, _app.level0));
    // MarquardtTolerance.
    let mt = cm.getMarquardtTolerance() || new (0, _control.MarquardtTolerance)(new Map(), (0, _app.big0));
    cm.setMarquardtTolerance(mt);
    createLabelWithInputForObject(mt, divCmDetails, (0, _app.boundary1), (0, _app.level0));
    // MarquardtDerivDelta.
    let mdd = cm.getMarquardtDerivDelta() || new (0, _control.MarquardtDerivDelta)(new Map(), (0, _app.big0));
 ...
 */ function processCalcMethodAnalyticalRepresentation(divCmDetails, cm) {
    // "me:format".
    let format = cm.getFormat() || new (0, _control.Format)(new Map(), (0, _control.Format).options[0]);
    ...
    processSelectElement((0, _control.Format), cm.getFormat.bind(cm), cm.setFormat.bind(cm), (0, _control.Format).tagName, (0, _control.Format).options);
    processSelectElement((0, _control.Format), ()=>format.getRateUnits(), format.setRateUnits.bind(format), (0, _control.Format).rateUnits, (0, _control.Format).rateUnitsOptions);
    processSelectElement((0, _control.Precision), cm.getPrecision.bind(cm), cm.setPrecision.bind(cm), (0, _control.Precision).tagName, (0, _mesmer.Mesmer).precisionOptions);
    ...
    processElement((0, _control.ChebNumTemp), cm.getChebNumTemp.bind(cm), cm.setChebNumTemp.bind(cm), (0, _control.ChebNumTemp).tagName);
    processElement((0, _control.ChebNumConc), cm.getChebNumConc.bind(cm), cm.setChebNumConc.bind(cm), (0, _control.ChebNumConc).tagName);
    processElement((0, _control.ChebMaxTemp), cm.getChebMaxTemp.bind(cm), cm.setChebMaxTemp.bind(cm), (0, _control.ChebMaxTemp).tagName);
    processElement((0, _control.ChebMinTemp), cm.getChebMinTemp.bind(cm), cm.setChebMinTemp.bind(cm), (0, _control.ChebMinTemp).tagName);
    processElement((0, _control.ChebMaxConc), cm.getChebMaxConc.bind(cm), cm.setChebMaxConc.bind(cm), (0, _control.ChebMaxConc).tagName);
    processElement((0, _control.ChebMinConc), cm.getChebMinConc.bind(cm), cm.setChebMinConc.bind(cm), (0, _control.ChebMinConc).tagName);
    processElement((0, _control.ChebTExSize), cm.getChebTExSize.bind(cm), cm.setChebTExSize.bind(cm), (0, _control.ChebTExSize).tagName);
    processElement((0, _control.ChebPExSize), cm.getChebPExSize.bind(cm), cm.setChebPExSize.bind(cm), (0, _control.ChebPExSize).tagName);
}
/**
 * @param divCmDetails The details div.
 ...
 */ function processCalcMethodThermodynamicTable(divCmDetails, cm) {
    ...
    processElement((0, _control.Tmin), cm.getTmin.bind(cm), cm.setTmin.bind(cm), (0, _control.Tmin).tagName);
    processElement((0, _control.Tmid), cm.getTmid.bind(cm), cm.setTmid.bind(cm), (0, _control.Tmid).tagName);
    processElement((0, _control.Tmax), cm.getTmax.bind(cm), cm.setTmax.bind(cm), (0, _control.Tmax).tagName);
    processElement((0, _control.Tstep), cm.getTstep.bind(cm), cm.setTstep.bind(cm), (0, _control.Tstep).tagName);
}
 ...
 */ function processCalcMethodSensitivityAnalysis(divCmDetails, cm) {
    ...
    processNumberElement((0, _control.SensitivityAnalysisSamples), cm.getSensitivityAnalysisSamples.bind(cm), cm.setSensitivityAnalysisSamples.bind(cm), (0, _control.SensitivityAnalysisSamples).tagName);
    processNumberElement((0, _control.SensitivityAnalysisOrder), cm.getSensitivityAnalysisOrder.bind(cm), cm.setSensitivityAnalysisOrder.bind(cm), (0, _control.SensitivityAnalysisOrder).tagName);
    processNumberElement((0, _control.SensitivityNumVarRedIters), cm.getSensitivityNumVarRedIters.bind(cm), cm.setSensitivityNumVarRedIters.bind(cm), (0, _control.SensitivityNumVarRedIters).tagName);
    // "me:sensitivityVarRedMethod".
    let sensitivityVarRedMethod = cm.getSensitivityVarRedMethod() || new (0, _control.SensitivityVarRedMethod)(new Map(), "");
    cm.setSensitivityVarRedMethod(sensitivityVarRedMethod);
    let tagName = (0, _control.SensitivityVarRedMethod).tagName;
    divCmDetails.appendChild((0, _html.createLabelWithSelect)(tagName, (0, _control.SensitivityVarRedMethod).options, tagName, (0, _control.SensitivityVarRedMethod).options[0], (0, _util.getID)(divCmDetails.id, tagName, "select"), (0, _app.boundary1), (0, _app.boundary1)));
 ...
 */ function createSelectElementCalcMethod(control, div, options, tagName, value, divCmDetailsId, divCmDetailsSelectId) {
    ...
    select.addEventListener("change", (event)=>{
        ...
        if (value == (0, _control.CalcMethodSimpleCalc).xsi_type || value == (0, _control.CalcMethodSimpleCalc).xsi_type2) // "me:simpleCalc", "simpleCalc".
        control.setCalcMethod(new (0, _control.CalcMethodSimpleCalc)(attributes));
        else if (value == (0, _control.CalcMethodGridSearch).xsi_type || value == (0, _control.CalcMethodGridSearch).xsi_type2) // "me:gridSearch", "gridSearch".
        control.setCalcMethod(new (0, _control.CalcMethodGridSearch)(attributes));
        else if (value == (0, _control.CalcMethodFitting).xsi_type || value == (0, _control.CalcMethodFitting).xsi_type2) {
            let cm = new (0, _control.CalcMethodFitting)(attributes);
            control.setCalcMethod(cm);
            processCalcMethodFitting(divCmDetails, cm);
        } else if (value == (0, _control.CalcMethodMarquardt).xsi_type || value == (0, _control.CalcMethodMarquardt).xsi_type2) {
            // "me:marquardt", "marquardt".
            let cm = new (0, _control.CalcMethodMarquardt)(attributes);
            control.setCalcMethod(cm);
            processCalcMethodMarquardt(divCmDetails, cm);
        } else if (value == (0, _control.CalcMethodAnalyticalRepresentation).xsi_type || value == (0, _control.CalcMethodAnalyticalRepresentation).xsi_type2) {
            // "me:analyticalRepresentation", "analyticalRepresentation".
            let cm = new (0, _control.CalcMethodAnalyticalRepresentation)(attributes);
            control.setCalcMethod(cm);
            processCalcMethodAnalyticalRepresentation(divCmDetails, cm);
        } else if (value == (0, _control.CalcMethodThermodynamicTable).xsi_type || value == (0, _control.CalcMethodThermodynamicTable).xsi_type2) {
            // "me:ThermodynamicTable", "ThermodynamicTable".
            let cm = new (0, _control.CalcMethodThermodynamicTable)(attributes);
            control.setCalcMethod(cm);
            processCalcMethodThermodynamicTable(divCmDetails, cm);
        } else if (value == (0, _control.CalcMethodSensitivityAnalysis).xsi_type || value == (0, _control.CalcMethodSensitivityAnalysis).xsi_type2) {
            // "me:sensitivityAnalysis", "sensitivityAnalysis".
            let cm = new (0, _control.CalcMethodSensitivityAnalysis)(new Map());
            control.setCalcMethod(cm);
...
},{"big.js":"91nMZ","./app":"dPB9w","./control":"Qx5gu","./html":"aLPSL","./mesmer":"kMp4Q","./util":"f0Rnl","./xml":"7znDa","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["8AHG6","dPB9w"], "dPB9w", "parcelRequirec042")
